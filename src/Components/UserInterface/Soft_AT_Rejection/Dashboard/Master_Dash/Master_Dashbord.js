...
const Master_Dashbord = () => {
    ...
    const fetchMasterData = async () => {
        ...
        const response = await postData('softat_rej/master_dashbord_api', formData)
        const responce1 = await postData('softat_rej/OverAllCircleWiseSummary', formData)
        const responce2 = await postData('softat_rej/overall_oem_wise_circle_wise_summary', formData)
        if(responce1){
    ...
    const oemCircleTable = async (data) => {
        ...
        const response = await postData('softat_rej/oem_wise_hyper_link_over_overall_circlewie', formData)
        if (response) {
            var arr = [];
    ...
    const fetchTableToggalData = async (props) => {
        ...
        const response = await postData('softat_rej/oem_wise_master_dashbord_api', formData)
    ...
    const showTableData = async (data) => {
        ...
        const response = await postData('softat_rej/oem_wise_site_details_master_dashbord', formData)
                ...
                return (
                    <>
                        <tr key={item} className={classes.hover} style={{ textAlign: "center", fontWeigth: 700 }}>
                            <td style={{ border: '1px solid black' }}>{item.circle}</td>
                            <td className={classes.hover} style={{ border: '1px solid black', cursor: 'pointer' }} onClick={() => { showTableData({ oem: 'Nokia', circle: item.circle }) }}>{item.Nokia}</td>
    ...
    const handleExport = () => {
        ...
        workbook.xlsx.writeBuffer().then(item => {
            ...
            anchor.href = url;
            anchor.download = "Soft_AT_Rejection.xlsx";
            anchor.click();
