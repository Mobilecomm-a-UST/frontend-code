...
const Make5G = () => {
  ...
  const handleSubmit = async () => {
    if (make4GFiles.length > 0 && selectCircle !== '') {
      ...
      const response = await postData('universal_alarm/upload_5g/', formData)
    ...
    <>
      <div style={{ margin: 5, marginLeft: 10 }}>
        <Breadcrumbs aria-label="breadcrumb" itemsBeforeCollapse={2} maxItems={3} separator={<KeyboardArrowRightIcon fontSize="small" />}>
          ...
          <Typography color='text.primary'>Make 5G Alarm</Typography>
        </Breadcrumbs>
      </div>
      <Slide
        direction='left'
              ...
              <Stack spacing={2} sx={{ marginTop: "-40px" }} direction={'column'}>
                ...
                <Box className={classes.Front_Box} >
                  <div className={classes.Front_Box_Hading}>
                    Select 5G Logs Files:-<span style={{ fontFamily: 'Poppins', color: "gray", marginLeft: 20 }}>{ }</span>
                  </div>
