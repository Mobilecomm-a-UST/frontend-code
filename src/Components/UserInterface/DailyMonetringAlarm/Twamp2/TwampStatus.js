...
const TwampStatus = () => {
        ...
        const handleSubmit = async () => {
            if (make4GFiles.length > 0) {
                ...
                const response = await postData('twamp2/twampdata/', formData)
    ...
    <>
                <div style={{ margin: 5, marginLeft: 10 }}>
                    <Breadcrumbs aria-label="breadcrumb" itemsBeforeCollapse={2} maxItems={3} separator={<KeyboardArrowRightIcon fontSize="small" />}>
                        ...
                        <Typography color='text.primary'>Twamp 2.0</Typography>
                    </Breadcrumbs>
                </div>
                <Slide
                    direction='left'
                        ...
                        <Box className={classes.main_Box}>
                            <Box className={classes.Back_Box} sx={{ width: { md: '75%', xs: '100%' } }}>
                                <Box className={classes.Box_Hading} >
                                    Create Twamp 2.0 Status
                                </Box>
                                <Stack spacing={2} sx={{ marginTop: "-40px" }} direction={'column'}>
    
                                    <Box className={classes.Front_Box} >
                                        <div className={classes.Front_Box_Hading}>
                                            Select Txt / Logs file:-<span style={{ fontFamily: 'Poppins', color: "gray", marginLeft: 20 }}>{ }</span>
                                        </div>
