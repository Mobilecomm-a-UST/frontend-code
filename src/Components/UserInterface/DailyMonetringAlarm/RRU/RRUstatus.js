    ...
    const handleSubmit = async () => {
        if (make4GFiles.length > 0) {
            ...
            for (let i = 0; i < make4GFiles.length; i++) {
                ...
                formData.append(`sdir_files`, make4GFiles[i]);
            }

            const response = await postData('dpr/sdir_status/', formData)
        ...
        <>
            <div style={{ margin: 5, marginLeft: 10 }}>
                <Breadcrumbs aria-label="breadcrumb" itemsBeforeCollapse={2} maxItems={3} separator={<KeyboardArrowRightIcon fontSize="small" />}>
                    ...
                    <Typography color='text.primary'>RRU Status</Typography>
                </Breadcrumbs>
            </div>
            <Slide
                direction='left'
            ...
            >
                <Box>
                    ...
                    <Box sx={{ display: download ? 'block' : 'none', textAlign: 'center' }}>
                        <a download href={link}><Button variant="outlined" onClick='' title="Export Excel" startIcon={<FileDownloadIcon style={{ fontSize: 30, color: "green" }} />} sx={{ marginTop: "10px", width: "auto" }}><span style={{ fontFamily: "Poppins", fontSize: "22px", fontWeight: 800, textTransform: "none", textDecorationLine: "none" }}>Download RRU Status</span></Button></a>
                    </Box>
                </Box>
            </Slide>
            {loading}
