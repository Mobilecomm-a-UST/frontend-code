...
const Relocation = () => {
    ...
    const FilterTableData = useMemo(() => {
        ...
        return filteredData?.map((item, index) => (
            ...
            >
                {/* <th style={{position: "sticky", left: 0, whiteSpace: 'nowrap', border: '1px solid black' }}>{index + 1}</th> */}
                <th style={{position: "sticky", left: 0,zIndex:1, whiteSpace: 'nowrap', border: '1px solid black'}} className={`${colorChange(item)}`}>{item.circle}</th>
                        ...
                        <table style={{ width: "100%", border: "1px solid black", borderCollapse: 'collapse', overflow: 'auto' }} >
                            <thead style={{ position: 'sticky', top: 0, zIndex: 2 }}>
                                <tr style={{ fontSize: 14, backgroundColor: "#223354", color: "white", border: '1px solid white' }}>
                                    {/* <th style={{position: "sticky", left: 0, padding: '1px 10px', whiteSpace: 'nowrap' }}>Sr. No.</th> */}
                                    <th style={{position: "sticky", left: 0,zIndex:6, backgroundColor: "#223354", padding: '1px 70px 1px 1px', whiteSpace: 'nowrap' }}>Circle <CheckPicker data={circleData.map(item => ({ label: item, value: item }))} value={selectCircle} onChange={(value) => { setSelectCircle(value) }} size="sm" appearance="default" placeholder="" style={{ width: 10 }} /> </th>
