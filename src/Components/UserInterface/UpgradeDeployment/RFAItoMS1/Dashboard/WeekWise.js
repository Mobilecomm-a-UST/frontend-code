...
const WeekWise = () => {
    ...
    const fetchDailyData = async () => {
        ...
        const res = await postData("upgrade_tracker/weekly_monthly_dashboard_file/", formData);
    ...
    const ClickDataGet = async (props) => {
        ...
        if (responce) {
            ...
            navigate(`/tools/upgrade_deployment/rfai_to_ms1_waterfall/${props.milestone}`)
        }
        else {
            action(false)
        }
