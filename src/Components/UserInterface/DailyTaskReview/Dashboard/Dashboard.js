...
import {
    ...
    LinearProgress, ToggleButton, ToggleButtonGroup, alpha, Skeleton, TextField,
    Table, TableBody, TableCell, TableHead, TableRow,
    // ADDED: for the user-click details popup
...
import KeyboardArrowRightIcon         from "@mui/icons-material/KeyboardArrowRight";
import PeopleOutlinedIcon             from "@mui/icons-material/PeopleOutlined";
// ADDED: icons for the new Task / Owner filters and their results table
import ChecklistOutlinedIcon          from "@mui/icons-material/ChecklistOutlined";
import PersonSearchOutlinedIcon       from "@mui/icons-material/PersonSearchOutlined";
import TableRowsOutlinedIcon          from "@mui/icons-material/TableRowsOutlined";
// ADDED: close icon for the user-details popup
...
const API = {
    ASSIGNED: "dailytask_review/analytics/assigned/",
    MYTASKS:  "dailytask_review/analytics/mytasks/",
    // ADDED: new endpoint for the Task / Owner filtered table
    TASKTABLE: "dailytask_review/analytics/get_task_table_data/",
};
...
// ─── User Workload Table — scrollable, hidden in My Tasks ────────────────────
// UPDATED: rows are now clickable — clicking a user calls onUserClick(userEmail)
// so the parent can open the details popup for that person.
const UserWorkloadTable = ({ rows, loading, onUserClick }) => {
    if (loading) return (
        <Box>
            {Array.from({length:5}).map((_,i)=><Skeleton key={i} height={52} sx={{ mb:0.5, borderRadius:2 }}/>)}
        </Box>
                    ...
                    {rows.map((u,idx)=>{
                        ...
                        return (
                            <TableRow
                                ...
                                sx={{ cursor:"pointer", "&:last-child td":{ border:0 }, "& td":{ borderBottom:"1px solid #f7f8fa", py:1.4 }, "&:hover":{ bgcolor:alpha(TEAL,0.05) } }}
                            >
                                <TableCell sx={{ pl:0 }}>
                                    <Box display="flex" alignItems="center" gap={1.2}>
                                        <Avatar sx={{ width:36, height:36, fontSize:11, fontWeight:700, bgcolor:alpha(avatarColor,0.15), color:avatarColor, border:`1.5px solid ${alpha(avatarColor,0.25)}`, flexShrink:0 }}>
...
// ─── ADDED: Task Table (results from get_task_table_data/, filtered by Task / Owner) ──
// Renders a generic table whose columns are derived from the keys of the first
// row returned by the API, so it works regardless of the exact result shape.
const TaskFilterResultsTable = ({ rows, loading }) => {
    if (loading) return (
        <Box>
            {Array.from({length:5}).map((_,i)=><Skeleton key={i} height={44} sx={{ mb:0.5, borderRadius:2 }}/>)}
        </Box>
    );
    if (!rows || rows.length===0) return (
        <Typography fontSize={12} color="text.disabled" py={4} textAlign="center">No matching tasks</Typography>
    );
        ...
        <Box sx={{ overflow:"auto", maxHeight:420,
            ...
            <Table size="small" stickyHeader sx={{ minWidth:600 }}>
                <TableHead>
                    <TableRow sx={{ "& th":{ borderBottom:"1px solid #f0f2f5", pb:1.2, pt:0.5, bgcolor:"#fff" } }}>
                        {columns.map((col)=>(
                            <TableCell key={col} sx={{ fontSize:12, fontWeight:600, color:"text.secondary", whiteSpace:"nowrap" }}>
                                {formatHeader(col)}
                            </TableCell>
                        ))}
                    </TableRow>
                ...
                <TableBody>
                    {rows.map((row,idx)=>(
                        <TableRow key={row.id ?? idx} sx={{ "&:last-child td":{ border:0 }, "& td":{ borderBottom:"1px solid #f7f8fa", py:1.2 }, "&:hover":{ bgcolor:alpha(TEAL,0.02) } }}>
                            {columns.map((col)=>(
                                <TableCell key={col} sx={{ fontSize:12.5, whiteSpace:"nowrap" }}>
                                    {String(row[col] ?? "—")}
                                </TableCell>
                            ))}
                        </TableRow>
...
// ─── ADDED: User details popup — shows only the requested fields
// (Task Id, Oem, Task, Status, Priority, Assigned By, Deadline, Remarks,
// Reassign Count, Is Overdue) for the clicked user's tasks, sourced from
// get_task_table_data/. ──
const UserDetailDialog = ({ open, onClose, userEmail, rows, loading }) => {
    return (
        <Dialog open={open} onClose={onClose} maxWidth="lg" fullWidth>
            <DialogTitle sx={{ display:"flex", alignItems:"center", justifyContent:"space-between", background:`linear-gradient(90deg,${TEAL_DARK} 0%,${TEAL} 100%)`, color:"#fff" }}>
                <Box display="flex" alignItems="center" gap={1}>
                    <PeopleOutlinedIcon sx={{ fontSize:20 }}/>
                    <Typography fontWeight={700} fontSize={16}>{userEmail ? getDisplayName(userEmail) : ""}'s Tasks</Typography>
                </Box>
            ...
            <DialogContent sx={{ p:0 }}>
                {loading ? (
                    <Box sx={{ p:2 }}>
                        {Array.from({length:4}).map((_,i)=><Skeleton key={i} height={44} sx={{ mb:0.5, borderRadius:2 }}/>)}
                    </Box>
                ) : (!rows || rows.length===0) ? (
                    <Typography fontSize={12} color="text.disabled" py={4} textAlign="center">No tasks found for this user</Typography>
                ) : (
                    <Box sx={{ overflow:"auto", maxHeight:480,
                        ...
                        <Table size="small" stickyHeader sx={{ minWidth:760 }}>
                            <TableHead>
                                <TableRow sx={{ "& th":{ borderBottom:"1px solid #f0f2f5", py:1, bgcolor:"#fff" } }}>
                                    <TableCell sx={{ fontSize:12, fontWeight:700, color:"text.secondary", whiteSpace:"nowrap" }}>Task Id</TableCell>
                                    <TableCell sx={{ fontSize:12, fontWeight:700, color:"text.secondary", whiteSpace:"nowrap" }}>Oem</TableCell>
                                    <TableCell sx={{ fontSize:12, fontWeight:700, color:"text.secondary", whiteSpace:"nowrap" }}>Task</TableCell>
                                    <TableCell sx={{ fontSize:12, fontWeight:700, color:"text.secondary", whiteSpace:"nowrap" }}>Status</TableCell>
                                    <TableCell sx={{ fontSize:12, fontWeight:700, color:"text.secondary", whiteSpace:"nowrap" }}>Priority</TableCell>
                                    <TableCell sx={{ fontSize:12, fontWeight:700, color:"text.secondary", whiteSpace:"nowrap" }}>Assigned By</TableCell>
                                    <TableCell sx={{ fontSize:12, fontWeight:700, color:"text.secondary", whiteSpace:"nowrap" }}>Deadline</TableCell>
                                    <TableCell sx={{ fontSize:12, fontWeight:700, color:"text.secondary", whiteSpace:"nowrap" }}>Remarks</TableCell>
                                    <TableCell align="center" sx={{ fontSize:12, fontWeight:700, color:"text.secondary", whiteSpace:"nowrap" }}>Reassign Count</TableCell>
                                    <TableCell align="center" sx={{ fontSize:12, fontWeight:700, color:"text.secondary", whiteSpace:"nowrap" }}>Is Overdue</TableCell>
                                </TableRow>
                            </TableHead>
                            <TableBody>
                                {rows.map((row,idx)=>(
                                    <TableRow key={row.task_id ?? idx} sx={{ "&:last-child td":{ border:0 }, "& td":{ borderBottom:"1px solid #f7f8fa", py:1.1 }, "&:hover":{ bgcolor:alpha(TEAL,0.02) } }}>
                                        <TableCell sx={{ fontSize:12.5, whiteSpace:"nowrap" }}>{row.task_id ?? "—"}</TableCell>
                                        <TableCell sx={{ fontSize:12.5, whiteSpace:"nowrap" }}>{row.oem ?? "—"}</TableCell>
                                        <TableCell sx={{ fontSize:12.5, maxWidth:220 }}>{row.task ?? "—"}</TableCell>
                                        <TableCell sx={{ fontSize:12.5, whiteSpace:"nowrap" }}>
                                            <Chip label={row.status ?? "—"} size="small" sx={{ height:20, fontSize:11, fontWeight:600, bgcolor:alpha(STATUS_COLORS[row.status]||TEAL,0.12), color:STATUS_COLORS[row.status]||TEAL_DARK }}/>
                                        </TableCell>
                                        <TableCell sx={{ fontSize:12.5, whiteSpace:"nowrap" }}>
                                            <Chip label={row.priority ?? "—"} size="small" sx={{ height:20, fontSize:11, fontWeight:600, bgcolor:alpha(PRIO_COLORS[row.priority]||TEAL,0.12), color:PRIO_COLORS[row.priority]||TEAL_DARK }}/>
                                        </TableCell>
                                        <TableCell sx={{ fontSize:12.5, whiteSpace:"nowrap" }}>{row.assigned_by ?? "—"}</TableCell>
                                        <TableCell sx={{ fontSize:12.5, whiteSpace:"nowrap" }}>{row.deadline ? fmtDate(row.deadline) : "—"}</TableCell>
                                        <TableCell sx={{ fontSize:12.5, maxWidth:200 }}>{row.remarks || "—"}</TableCell>
                                        <TableCell align="center" sx={{ fontSize:12.5 }}>{row.reassign_count ?? 0}</TableCell>
                                        <TableCell align="center">
                                            {row.is_overdue
                                                ? <Chip label="Yes" size="small" sx={{ height:20, fontSize:11, fontWeight:700, bgcolor:"#fdecea", color:"#c62828" }}/>
                                                : <Chip label="No" size="small" sx={{ height:20, fontSize:11, fontWeight:700, bgcolor:"#e8f5e9", color:"#2e7d32" }}/>
                                            }
                                        </TableCell>
                                    </TableRow>
                                ))}
...
const Dashboard = () => {
    ...
    // ADDED: Task / Owner filter state + results for the new get_task_table_data/ API
    const [taskFilter, setTaskFilter] = useState("");
    const [ownerFilter, setOwnerFilter] = useState("");
    const [taskTableRows, setTaskTableRows] = useState([]);
    const [taskTableMeta, setTaskTableMeta] = useState(null);
    const [taskTableLoading, setTaskTableLoading] = useState(false);

    // ADDED: Task / Owner filter specifically for the User Workload table
    const [uwTaskFilter, setUwTaskFilter] = useState("");
    const [uwOwnerFilter, setUwOwnerFilter] = useState("");
    const [uwFilteredOwners, setUwFilteredOwners] = useState(null); // null = no task-based filter active
    const [uwFilterLoading, setUwFilterLoading] = useState(false);

    // ADDED: user-click details popup state
    ...
    // ADDED: fetches the Task / Owner filtered table from get_task_table_data/
    // using the same assigned_by (userEmail) and date range as the rest of
    // the dashboard, plus the new "task" and "owner" filter fields.
    const fetchTaskTable = useCallback(async () => {
        if (!userEmail) return;
        setTaskTableLoading(true);
        try {
            const headers = { "Content-Type":"application/json" };
            const res = await fetch(`${BASE_URL}${API.TASKTABLE}`, {
                method: "POST",
                headers,
                body: JSON.stringify({
            ...
            setTaskTableRows(Array.isArray(data?.results) ? data.results : []);
            setTaskTableMeta(data?.meta || null);
        } catch (err) {
            console.error("Dashboard fetchTaskTable:", err);
            setTaskTableRows([]);
            setTaskTableMeta(null);
        } finally {
            setTaskTableLoading(false);
        }
    }, [userEmail, dateFrom, dateTo, taskFilter, ownerFilter]);

    // ADDED: when a Task filter is typed for the User Workload table, look up
    // which owners have matching tasks via get_task_table_data/, so the
    // workload rows can be narrowed down to just those users.
    const fetchUwTaskOwners = useCallback(async () => {
        if (!userEmail || !uwTaskFilter) { setUwFilteredOwners(null); return; }
        setUwFilterLoading(true);
        try {
            const headers = { "Content-Type":"application/json" };
            const res = await fetch(`${BASE_URL}${API.TASKTABLE}`, {
                method: "POST",
            ...
            results.forEach((r) => {
                if (Array.isArray(r.owner)) r.owner.forEach((o)=>owners.add(o));
                else if (r.owner) owners.add(r.owner);
                if (r.assigned_by) owners.add(r.assigned_by);
            });
            setUwFilteredOwners(owners);
        } catch (err) {
            console.error("Dashboard fetchUwTaskOwners:", err);
            setUwFilteredOwners(new Set());
        } finally {
            setUwFilterLoading(false);
        }
    }, [userEmail, dateFrom, dateTo, uwTaskFilter]);

    // ADDED: fetches a single user's task details (only the fields requested)
    // for the details popup, filtered by owner = that user's email.
    const fetchUserDetail = useCallback(async (ownerEmail) => {
        if (!userEmail || !ownerEmail) return;
        ...
        try {
            ...
            const data = await res.json().catch(() => ({}));
            setUserDetailRows(Array.isArray(data?.results) ? data.results : []);
        } catch (err) {
            console.error("Dashboard fetchUserDetail:", err);
            setUserDetailRows([]);
        } finally {
    ...
    // ADDED: opens the popup for a clicked user and loads their task details
    const handleUserClick = useCallback((ownerEmail) => {
        setSelectedUserEmail(ownerEmail);
        setUserDetailOpen(true);
        fetchUserDetail(ownerEmail);
    }, [fetchUserDetail]);

    useEffect(()=>{ const t=setTimeout(()=>fetchAll(),400); return ()=>clearTimeout(t); },[fetchAll]);

    // ADDED: debounce Task/Owner table fetch same way the rest of the dashboard does
    useEffect(()=>{ const t=setTimeout(()=>fetchTaskTable(),400); return ()=>clearTimeout(t); },[fetchTaskTable]);

    // ADDED: debounce the User Workload task-filter lookup
    useEffect(()=>{ const t=setTimeout(()=>fetchUwTaskOwners(),400); return ()=>clearTimeout(t); },[fetchUwTaskOwners]);
    ...
    // ADDED: userPerf rows narrowed by the User Workload Task / Owner filters.
    // Owner filter matches on the email text directly (client-side).
    // Task filter matches against the set of owners resolved via
    // fetchUwTaskOwners (users who have at least one task matching the text).
    const displayedUserPerf = useMemo(() => {
        return userPerf.filter((u) => {
            const matchesOwner = !uwOwnerFilter || u.user.toLowerCase().includes(uwOwnerFilter.toLowerCase());
            const matchesTask  = !uwTaskFilter || (uwFilteredOwners && uwFilteredOwners.has(u.user));
            return matchesOwner && matchesTask;
        });
    }, [userPerf, uwOwnerFilter, uwTaskFilter, uwFilteredOwners]);
            ...
            <Paper elevation={0} sx={{ borderRadius:"20px", overflow:"hidden", border:"1px solid #e8ecf0", bgcolor:"#fff", boxShadow:"0 2px 20px rgba(0,0,0,0.07)" }}>
                ...
                <Box sx={{ px:3, pt:3, pb:2.5, borderBottom:"1px solid #f0f2f5", background:`linear-gradient(135deg,#fff 60%,${alpha(TEAL,0.03)} 100%)` }}>
                    <Box display="flex" alignItems="center" justifyContent="space-between" flexWrap="wrap" gap={2}>
                        ...
                        <Box display="flex" gap={1.5} alignItems="center" flexWrap="wrap">
                            ...
                            {/* Task / Owner filters for the get_task_table_data/ table below */}
                            {/* <Box display="flex" alignItems="center" gap={1}>
                                <TextField size="small" label="Task" placeholder="e.g. 2G site ka Data chahiye"
                                    value={taskFilter} onChange={(e)=>setTaskFilter(e.target.value)}
                                    sx={{ minWidth:180, "& .MuiOutlinedInput-root":{ borderRadius:"10px", "&:hover fieldset":{ borderColor:TEAL }, "&.Mui-focused fieldset":{ borderColor:TEAL } }, "& label.Mui-focused":{ color:TEAL } }}/>
                                <TextField size="small" label="Owner" placeholder="owner@ust.com"
                            ...
                            <Button variant="outlined"
                                startIcon={<RefreshIcon sx={{ fontSize:"17px !important", animation:refreshing?"spin .8s linear infinite":"none", "@keyframes spin":{ to:{ transform:"rotate(360deg)" } } }}/>}
                                onClick={()=>{ fetchAll(true); fetchTaskTable(); }} disabled={refreshing}
                                sx={{ textTransform:"none", fontWeight:600, borderRadius:"10px", fontSize:13, px:2, height:36, borderColor:TEAL_MID, color:TEAL, "&:hover":{ borderColor:TEAL, bgcolor:alpha(TEAL,0.05) } }}>
                                {refreshing?"…":"Refresh"}
                            </Button>
                        </Box>
                ...
                <Box sx={{ p:3 }}>
                    ...
                    */}
                    ...
                    <Box display="grid" sx={{ gridTemplateColumns: showUserWorkload ? "1.7fr 1fr" : "1fr", gap:2 }}>
                        ...
                        {showUserWorkload && (
                            <Paper elevation={0} sx={{ p:2.5, borderRadius:"14px", border:"1px solid #e8ecf0" }}>
                                ...
                                {/* ADDED: Task / Owner filter for the User Workload table */}
                                <Box display="flex" gap={1} flexWrap="wrap" mb={1.5}>
                                    <TextField size="small" label="Filter by Task" placeholder="e.g. 2G site ka Data"
                                        value={uwTaskFilter} onChange={(e)=>setUwTaskFilter(e.target.value)}
                                        sx={{ minWidth:180, "& .MuiOutlinedInput-root":{ borderRadius:"10px", "&:hover fieldset":{ borderColor:TEAL }, "&.Mui-focused fieldset":{ borderColor:TEAL } }, "& label.Mui-focused":{ color:TEAL } }}/>
                                    <TextField size="small" label="Filter by Owner" placeholder="owner@ust.com"
                                        value={uwOwnerFilter} onChange={(e)=>setUwOwnerFilter(e.target.value)}
                                        sx={{ minWidth:180, "& .MuiOutlinedInput-root":{ borderRadius:"10px", "&:hover fieldset":{ borderColor:TEAL }, "&.Mui-focused fieldset":{ borderColor:TEAL } }, "& label.Mui-focused":{ color:TEAL } }}/>
                                    {uwFilterLoading && <Chip label="Filtering…" size="small" sx={{ height:24, fontSize:11, alignSelf:"center", bgcolor:alpha(TEAL,0.08), color:TEAL_DARK }}/>}
                                </Box>

                                <Typography fontSize={11} color="text.disabled" mb={1}>Click a user to see their task details</Typography>

                                <UserWorkloadTable rows={displayedUserPerf} loading={loading} onUserClick={handleUserClick}/>
                            </Paper>
                        )}
                    ...
                    {/* ── Row 4 — Task / Owner filtered results table (get_task_table_data/) ── */}
                    {/* <Box mt={2}>
                        <Paper elevation={0} sx={{ p:2.5, borderRadius:"14px", border:"1px solid #e8ecf0" }}>
                            <Box display="flex" alignItems="center" justifyContent="space-between" mb={1.5}>
                                <SectionTitle icon={<TableRowsOutlinedIcon/>} title="Task lookup (filter by task / owner)"/>
                                {taskTableMeta?.total_records !== undefined && (
                                    <Chip label={`${taskTableMeta.total_records} records`} size="small"
                                        sx={{ height:22, fontSize:11.5, fontWeight:600, bgcolor:alpha(TEAL,0.08), color:TEAL_DARK, border:`1px solid ${TEAL_MID}` }}/>
                                )}
                            </Box>
                            <Box display="flex" gap={1} flexWrap="wrap" mb={2}>
                                {taskFilter && (
                                    <Chip icon={<ChecklistOutlinedIcon sx={{ fontSize:"13px !important" }}/>}
                                        label={`Task: ${taskFilter}`} size="small"
                                        onDelete={()=>setTaskFilter("")}
                                        sx={{ height:24, fontSize:11.5, fontWeight:600, bgcolor:alpha(TEAL,0.08), color:TEAL_DARK, border:`1px solid ${TEAL_MID}` }}/>
                                )}
                                {ownerFilter && (
                                    <Chip icon={<PersonSearchOutlinedIcon sx={{ fontSize:"13px !important" }}/>}
                                        label={`Owner: ${ownerFilter}`} size="small"
                                        onDelete={()=>setOwnerFilter("")}
                                        sx={{ height:24, fontSize:11.5, fontWeight:600, bgcolor:alpha(TEAL,0.08), color:TEAL_DARK, border:`1px solid ${TEAL_MID}` }}/>
                                )}
                            </Box>
                            <TaskFilterResultsTable rows={taskTableRows} loading={taskTableLoading}/>
                        </Paper>
                    </Box>
 */}
                </Box>
            ...
            {/* ADDED: user-click details popup showing only the requested fields */}
            <UserDetailDialog
                open={userDetailOpen}
...
//                                     sx={{ minWidth:148, "& .MuiOutlinedInput-root":{ borderRadius:"10px", "&:hover fieldset":{ borderColor:TEAL }, "&.Mui-focused fieldset":{ borderColor:TEAL } }, "& label.Mui-focused":{ color:TEAL } }}/>
//                             </Box>
//                             {/* ADDED: Task / Owner filters for the get_task_table_data/ table below */}
//                             <Box display="flex" alignItems="center" gap={1}>
//                                 <TextField size="small" label="Task" placeholder="e.g. 2G site ka Data chahiye"
//                                     value={taskFilter} onChange={(e)=>setTaskFilter(e.target.value)}
//                                     sx={{ minWidth:180, "& .MuiOutlinedInput-root":{ borderRadius:"10px", "&:hover fieldset":{ borderColor:TEAL }, "&.Mui-focused fieldset":{ borderColor:TEAL } }, "& label.Mui-focused":{ color:TEAL } }}/>
...
//                     {/* ── ADDED: Row 4 — Task / Owner filtered results table (get_task_table_data/) ── */}
//                     <Box mt={2}>
//                         <Paper elevation={0} sx={{ p:2.5, borderRadius:"14px", border:"1px solid #e8ecf0" }}>
//                             <Box display="flex" alignItems="center" justifyContent="space-between" mb={1.5}>
//                                 <SectionTitle icon={<TableRowsOutlinedIcon/>} title="Task lookup (filter by task / owner)"/>
