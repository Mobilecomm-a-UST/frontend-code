...
// // Recurrence type chips — maps directly to recurrence_rule.type values
// // NOTE: "once" removed from selectable options per request. Daily is now the default.
// const RECURRENCE_TYPES = [
//     { value: "daily", label: "Daily", icon: "📅" },
//     { value: "weekly", label: "Weekly", icon: "🗓️" },
//     { value: "monthly", label: "Monthly", icon: "📆" },
...
// // Always returns a COMPLETE set of form fields (with sensible defaults for
// // fields that don't apply to the stored type). This is what prevents the
// // "Cannot read properties of undefined (reading 'includes'/'map')" crashes
// // when a user opens Edit on a template and then switches recurrence type.
// const parseRecurrenceRule = (rule) => {
//     const base = {
...
//     switch (rule.type) {
//         case "daily":
//             return { ...base, recurrenceType: "daily", recurrenceInterval: rule.interval ?? 1, times };
//         case "weekly":
//             return { ...base, recurrenceType: "weekly", recurrenceInterval: rule.interval ?? 1, recurrenceDays: rule.days ?? [], times };
//         case "monthly":
//             return rule.dates
//                 ? { ...base, recurrenceType: "monthly", recurrenceInterval: rule.interval ?? 1, monthlyMode: "date", monthlyDates: rule.dates ?? [], times }
//                 : { ...base, recurrenceType: "monthly", recurrenceInterval: rule.interval ?? 1, monthlyMode: "weekday", monthlyWeek: rule.week ?? "FIRST", monthlyDay: rule.day ?? "MON", times };
//         case "custom":
//             return { ...base, recurrenceType: "custom", recurrenceDays: rule.days ?? [], times };
//         case "once":
//             // Legacy data saved before "once" was removed from the UI —
//             // treat it as a one-off daily rule so a valid tab is highlighted.
//             return { ...base, recurrenceType: "daily", recurrenceInterval: 1, times };
//         default:
//             return { ...base, times };
//     }
// };
...
//     const timesStr = (rule.times || []).map(fmtTime).join(", ");
//     switch (rule.type) {
//         case "once": return `Once · ${timesStr}`; // kept for display of legacy records only
//         case "daily": return rule.interval > 1 ? `Every ${rule.interval} days · ${timesStr}` : `Daily · ${timesStr}`;
//         case "weekly": {
//             const days = (rule.days || []).join(", ");
//             return rule.interval > 1 ? `Every ${rule.interval} weeks on ${days} · ${timesStr}` : `Weekly on ${days} · ${timesStr}`;
...
// // Same fix as parseRecurrenceRule: always return every field with a default
// // so switching Deadline Type in the edit dialog never hits an undefined value.
// const parseDeadlineRule = (rule) => {
//     const base = {
//         deadlineType: "same_day",
//         deadlineTime: "18:00",
...
//     if (!rule) return base;
//     switch (rule.type) {
//         case "after_hours": return { ...base, deadlineType: "after_hours", deadlineHours: rule.hours ?? 6 };
//         case "after_days": return { ...base, deadlineType: "after_days", deadlineDays: rule.days ?? 3, deadlineTime: rule.time ?? "18:00" };
//         case "after_weeks": return { ...base, deadlineType: "after_weeks", deadlineWeeks: rule.weeks ?? 2, deadlineTime: rule.time ?? "18:00" };
//         case "fixed_date": return { ...base, deadlineType: "fixed_date", deadlineDate: rule.date ?? "", deadlineTime: rule.time ?? "18:00" };
//         case "same_day":
//         default:
//             return { ...base, deadlineType: "same_day", deadlineTime: rule.time ?? "18:00" };
//     }
// };
...
// const DaysToggleGroup = ({ days, onChange }) => {
//     const safeDays = days ?? [];
//     const toggle = (d) => onChange(safeDays.includes(d) ? safeDays.filter((x) => x !== d) : [...safeDays, d]);
//     return (
//         <Box display="flex" gap={0.7} flexWrap="wrap">
//             {DAYS_OF_WEEK.map((d) => {
//                 const active = safeDays.includes(d);
//                 return (
//                     <Box key={d} onClick={() => toggle(d)} sx={{
//                         px: 1.4, py: 0.5, borderRadius: "8px", fontSize: 12, fontWeight: 700, cursor: "pointer",
//                         border: `1.5px solid ${active ? TEAL : "#e0e0e0"}`,
...
//     const [temp, setTemp] = useState("09:00");
//     const add = () => {
//         if (temp && !safeTimes.includes(temp)) onChange([...safeTimes, temp].sort());
//     };
//     const remove = (t) => onChange(safeTimes.filter((x) => x !== t));
//     return (
//         <Box>
//             <Box display="flex" gap={1} alignItems="center" mb={1}>
//                 <TextField type="time" size="small" value={temp} onChange={(e) => setTemp(e.target.value)}
...
//                         sx={{ bgcolor: TEAL_LIGHT, color: TEAL_DARK, fontWeight: 600, fontSize: 11.5, "& .MuiChip-deleteIcon": { color: TEAL } }} />
//                 ))}
//                 {safeTimes.length === 0 && <Typography fontSize={11} color="text.disabled">No times added yet</Typography>}
//             </Box>
//         </Box>
//     );
// };
...
//     const add = () => {
//         const n = Number(temp);
//         if (n >= 1 && n <= 31 && !safeDates.includes(n)) onChange([...safeDates, n].sort((a, b) => a - b));
//         setTemp("");
//     };
//     const remove = (n) => onChange(safeDates.filter((x) => x !== n));
//     return (
//         <Box>
//             <Box display="flex" gap={1} alignItems="center" mb={1}>
//                 <TextField type="number" size="small" placeholder="e.g. 5" value={temp}
...
//                         sx={{ bgcolor: TEAL_LIGHT, color: TEAL_DARK, fontWeight: 600, fontSize: 11.5, "& .MuiChip-deleteIcon": { color: TEAL } }} />
//                 ))}
//                 {safeDates.length === 0 && <Typography fontSize={11} color="text.disabled">No dates added yet</Typography>}
//             </Box>
//         </Box>
//     );
// };
...
//                     <Chip label={row.priority || "—"} size="small"
//                         sx={{ bgcolor: pm.bg, color: pm.color, fontWeight: 700, fontSize: 10, height: 18, border: `1px solid ${alpha(pm.color, 0.3)}` }} />
//                     {/* The "# id" chip has been removed per request */}
//                 </Box>
//                 <Box display="flex" gap={0.3} alignItems="center">
//                     <Tooltip title={active ? "Deactivate template" : "Activate template"} arrow>
//                         <IconButton size="small" onClick={() => onToggleActive(row)} sx={{ p: 0.3, color: active ? "#2e7d32" : "#9e9e9e" }}>
...
//     useEffect(() => {
//         if (open) {
//             // Always merge onto EMPTY_FORM so every field has a defined value,
//             // even if initialForm (from openEdit) only contains a subset.
...
//     const openEdit = (row) => {
//         setEditId(row.id);
//         // Start from EMPTY_FORM so every field (recurrenceDays, monthlyDates,
//         // monthlyWeek/Day, deadlineHours/Days/Weeks/Date, etc.) is always
//         // defined — then layer the row's actual values on top. This is what
//         // prevents "Cannot read properties of undefined (reading 'includes'/'map')"
//         // when switching recurrence/deadline type inside the edit dialog.
//         setEditForm({
//             ...EMPTY_FORM,
//             templateName: row.template_name ?? "",
//             task: row.task ?? "",
...
// // ═════════════════════════════════════════════════════════════════════════
// // HELP TOOLTIP — small "i" icon that reveals a rich explanation on hover
// // ═════════════════════════════════════════════════════════════════════════
// const SectionHelpTooltip = ({ title, items, color = TEAL }) => (
//     <Tooltip
//         arrow
...
//         title={
//             <Box>
//                 <Typography fontSize={12.5} fontWeight={700} mb={0.8} sx={{ color: "#fff" }}>
//                     {title}
//                 </Typography>
//                 <Box display="flex" flexDirection="column" gap={0.8}>
//                     {items.map((it) => (
//                         <Box key={it.label} display="flex" gap={0.9}>
//                             <Typography fontSize={11.5} fontWeight={700} sx={{ color: alpha("#fff", 0.95), minWidth: 62, flexShrink: 0 }}>
//                                 {it.label}
//                             </Typography>
//                             <Typography fontSize={11.5} sx={{ color: alpha("#fff", 0.78), lineHeight: 1.4 }}>
//                                 {it.text}
//                             </Typography>
//                         </Box>
//                     ))}
...
//     >
//         <InfoOutlinedIcon
//             sx={{ fontSize: 15, color: alpha(color, 0.7), cursor: "help", "&:hover": { color } }}
//         />
//     </Tooltip>
// );

// // Content for the Recurring (Template Deadline / Schedule) section
// const RECURRENCE_HELP_ITEMS = [
//     { label: "Daily", text: "Repeats every N day(s) at the time(s) you set. \"Every 1 day\" = every single day, \"Every 2 days\" = alternate days, and so on." },
//     { label: "Weekly", text: "Pick specific weekdays (e.g. MON, WED, FRI) and it repeats on those days every N week(s), at the time(s) you set." },
//     { label: "Monthly", text: "Two modes — \"By date of month\" fires on chosen day numbers (e.g. 1st & 15th) every N month(s). \"By weekday pattern\" fires on things like \"FIRST MON\" or \"LAST FRI\" every N month(s)." },
//     { label: "Custom", text: "Pick any combination of weekdays with no week interval — useful for irregular but fixed weekly patterns (e.g. only TUE & THU, every week)." },
//     { label: "Time(s)", text: "One or more times of day the task should be generated/assigned on the days that match the rule above." },
// ];

// // Content for the Deadline Rule (Task Deadline) section
// const DEADLINE_HELP_ITEMS = [
//     { label: "Same Day", text: "The task is due the same day it's assigned, by the time you set (e.g. 6:00 PM)." },
//     { label: "After Hours", text: "The task is due a fixed number of hours after it's assigned — e.g. 6 hours after assignment, regardless of clock time." },
//     { label: "After Days", text: "The task is due N calendar days after it's assigned, by the time you set on that day." },
//     { label: "After Weeks", text: "The task is due N calendar weeks after it's assigned, by the time you set on that day." },
//     { label: "Fixed Date", text: "The task is always due on one specific calendar date and time you choose, no matter when it was assigned — good for one-off compliance deadlines." },
// ];
...
//                             <RepeatOutlinedIcon sx={{ fontSize: 16, color: TEAL }} />
//                             <Typography fontSize={12.5} fontWeight={700} color={TEAL_DARK}>
//                                 Template Deadline
//                             </Typography>
//                             <SectionHelpTooltip
//                                 title="How recurrence works"
//                                 items={RECURRENCE_HELP_ITEMS}
//                                 color={TEAL}
//                             />
//                         </Box>
...
//                         <Box display="flex" alignItems="center" gap={0.6} mb={1.5}>
//                             <HourglassBottomOutlinedIcon sx={{ fontSize: 16, color: "#e65100" }} />
//                             <Typography fontSize={12.5} fontWeight={700} color="#9a3412">
//                                 Task Deadline
//                             </Typography>
//                             <SectionHelpTooltip
//                                 title="How deadlines work"
//                                 items={DEADLINE_HELP_ITEMS}
//                                 color="#e65100"
//                             />
//                         </Box>
...
// Handles the common DRF-style shapes the backend sends back, e.g.:
//   { "template_name": ["task template with this template name already exists."] }
//   { "detail": "Not found." }
//   { "message": "Something went wrong" }
// Returns the first human-readable string found, or "" if nothing usable.
...
const apiFetch = async (endpoint, method = "GET", body = null) => {
    ...
    if (body) options.body = JSON.stringify(body);
    const res = await fetch(url, options);
    // Always try to parse JSON — even on error responses — so we can surface
    // the backend's actual validation message (e.g. duplicate template name)
    // instead of a generic "API error 400".
    const data = await res.json().catch(() => ({}));
    if (!res.ok) {
        const message = extractApiErrorMessage(data) || `API error ${res.status}`;
...
// CREATE / EDIT TEMPLATE DIALOG
// ═════════════════════════════════════════════════════════════════════════
// `existingTemplates` lets us do an instant, client-side duplicate-name
// check (no network round trip) — while the backend's actual response is
// still respected as the source of truth via handleSave's catch block below.
const CreateTemplateDialog = ({ open, onClose, editId, initialForm, onSaved, userEmail, existingTemplates = [] }) => {
    const [form, setForm] = useState(EMPTY_FORM);
    const [errors, setErrors] = useState({});
    const [saving, setSaving] = useState(false);
    ...
    // ── Duplicate-name check (case-insensitive, whitespace-trimmed) ──
    // Excludes the template currently being edited so re-saving an unchanged
    // name doesn't false-positive against itself.
    const isDuplicateTemplateName = (name) => {
        const target = name.trim().toLowerCase();
        if (!target) return false;
        return existingTemplates.some(
            (t) => t.id !== editId && (t.template_name || "").trim().toLowerCase() === target
        );
    };

    // Runs on every keystroke so the user sees the warning immediately,
    // instead of only finding out after clicking "Create Template".
    const handleTemplateNameChange = (value) => {
        setForm((p) => ({ ...p, templateName: value }));
        setErrors((e) => ({
            ...e,
            templateName: isDuplicateTemplateName(value) ? "A template with this name already exists try a different name" : "",
        }));
    };
    ...
    const validate = () => {
        const e = {};
        if (!form.templateName.trim()) e.templateName = "Template name is required";
        else if (isDuplicateTemplateName(form.templateName)) e.templateName = "A template with this name already exists try a different name";
        if (!form.task.trim()) e.task = "Task name is required";
        if (form.owners.length === 0) e.owners = "Add at least one owner email";
        if (!form.startDate) e.startDate = "Start date is required";
        if (form.recurrenceType === "weekly" && form.recurrenceDays.length === 0) e.recurrenceDays = "Select at least one day";
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            console.error("handleSave error:", err);
            // If the backend flagged a specific field (e.g. template_name),
            // surface it right under that field too — not just as a toast —
            // so it matches the same in-form experience as the other
            ...
            if (err.data && typeof err.data === "object") {
                ...
                Object.entries(err.data).forEach(([key, val]) => {
                    ...
                    if (formField && typeof msg === "string") newFieldErrors[formField] = msg;
                });
                if (Object.keys(newFieldErrors).length) setErrors((e) => ({ ...e, ...newFieldErrors }));
            }
    ...
    return (
        <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth disableScrollLock
            ...
            <DialogContent sx={{ px: 3, pt: 2.5, pb: 1.5, overflowY: "auto" }}>
                <Box display="flex" flexDirection="column" gap={2.2}>
                    ...
                    <TextField label="Template Name" value={form.templateName}
                        ...
                        helperText={errors.templateName || "Unique name to identify this template"}
                        size="small" fullWidth sx={fieldSx} placeholder="e.g. Huawei Weekday KPI Check"
                        InputProps={{ startAdornment: (<InputAdornment position="start"><LibraryAddOutlinedIcon sx={{ fontSize: 18, color: TEAL }} /></InputAdornment>) }} />
            ...
            <DialogActions sx={{ px: 3, pb: 2.5, pt: 1.5, gap: 1 }}>
                ...
                <Button variant="contained" onClick={handleSave} disabled={saving || !!errors.templateName}
                    startIcon={saving ? <CircularProgress size={15} color="inherit" /> : <LibraryAddOutlinedIcon sx={{ fontSize: 16 }} />}
                    sx={{ bgcolor: TEAL, "&:hover": { bgcolor: TEAL_DARK }, textTransform: "none", fontWeight: 700, borderRadius: "10px", px: 3, boxShadow: `0 2px 10px ${alpha(TEAL, 0.4)}`, fontSize: 14 }}>
                    {saving ? "Saving…" : editId ? "Update Template" : "Create Template"}
                </Button>
