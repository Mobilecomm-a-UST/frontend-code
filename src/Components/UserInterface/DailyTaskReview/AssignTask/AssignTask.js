...
// // ── Status icons for table Status column ──────────────────────────────────────
...
//     REASSIGN: (pk) => `dailytask_review/assign_task/reassign/${pk}/`,
...
// // ── Status icon helper for table Status column ────────────────────────────────
// const StatusIcon = ({ status }) => {
//     const map = {
//         "Active":      { icon: <RadioButtonCheckedOutlinedIcon sx={{ fontSize: 15 }} />, color: "#f57c00" },
//         "In Progress": { icon: <SyncOutlinedIcon sx={{ fontSize: 15 }} />,               color: "#2e7d32" },
//         "Completed":   { icon: <CheckCircleIcon sx={{ fontSize: 15 }} />,                color: "#1565c0" },
//         "Cancelled":   { icon: <DoDisturbAltOutlinedIcon sx={{ fontSize: 15 }} />,       color: "#c62828" },
//     };
//     const meta = map[status] ?? { icon: <RadioButtonCheckedOutlinedIcon sx={{ fontSize: 15 }} />, color: TEAL };
//     return (
...
//             <Typography fontSize={11} fontWeight={700} color={meta.color} sx={{ whiteSpace: "nowrap" }}>
//                 {status ?? "Active"}
//             </Typography>
//         </Box>
//     );
...
//                     )}
//                     {row.reassign_count > 0 && (
//                         <Chip
//                             icon={<SyncAltOutlinedIcon sx={{ fontSize: "11px !important", color: "#0d47a1 !important" }} />}
//                             label={row.reassign_count} size="small"
//                             sx={{ bgcolor: alpha("#0d47a1", 0.08), color: "#0d47a1", fontWeight: 700, fontSize: 10, height: 18 }} />
//                     )}
//                 </Box>
...
// // ASSIGN TASK DIALOG
// // ═════════════════════════════════════════════════════════════════════════════
// const AssignTaskDialog = ({ open, onClose, editId, taskId, initialForm, onSaved, taskOptionsList, userEmail, userID }) => {
...
//     // ── Owner Email: single-selection only — selecting a new owner replaces the previous one ──
...
//         setSaving(true);
//         try {
//             const { taskName, payload } = buildPayload();
...
//     // ── Reassign Task: POST /assign_task/reassign/<pk>/ ────────────────────────
...
//         try {
//             const { taskName, payload } = buildPayload();
//             // Reassign endpoint expects the same shape as update, without assigned_at
//             const { assigned_at, ...reassignPayload } = payload;
//             await apiFetch(API.REASSIGN(editId), "POST", reassignPayload);
...
//                 html: `<b>${taskName}</b> reassigned to <b>${form.recipients.map(r => r.email).join(", ")}</b>`,
//                 timer: 2800, showConfirmButton: false, timerProgressBar: true,
//             });
//         } catch (err) {
//             console.error("handleReassign error:", err);
//             Swal.fire("Error", `Failed to reassign task: ${err.message}`, "error");
//         } finally {
...
//                             </Typography>
//                             {/* ── Task ID — small, clean, above the form ── */}
//                             {editId && taskId && (
//                                 <Chip
//                                     icon={<TagOutlinedIcon sx={{ fontSize: "12px !important", color: "#5b21b6 !important" }} />}
//                                     label={taskId}
//                                     size="small"
//                                     sx={{
//                                         mt: 0.6, height: 20, fontSize: 10.5, fontWeight: 700, fontFamily: "monospace",
//                                         bgcolor: alpha("#7c3aed", 0.08), color: "#5b21b6",
...
//                     {/* ── Owner Email (single selection only) ─────────────────────────── */}
//                     <Box>
...
//                             Owner Email
//                             <Chip
//                                 label={form.recipients.length > 0 ? "1 selected" : "0 selected"}
//                                 size="small"
//                                 sx={{
...
//                                         helperText={
//                                             errors.recipients ||
//                                             "Search from saved emails or type a new one · Enter to select · Only one owner allowed"
//                                         }
//                                         sx={fieldSx}
//                                         InputProps={{
//                                             ...params.InputProps,
...
//                                             endAdornment: (
//                                                 <>
//                                                     {emailLoading ? <CircularProgress color="inherit" size={14} /> : null}
//                                                     {params.InputProps.endAdornment}
//                                                 </>
//                                             ),
//                                             onKeyDown: (e) => {
...
//                                 Remove the selected owner above to choose a different one.
//                             </Typography>
//                         )}
//                     </Box>
...
//                 {editId && (
//                     <Tooltip title="Send this task's current details to a new owner via the reassign API" arrow>
//                         <span>
//                             <Button variant="outlined" onClick={handleReassign} disabled={reassigning || saving}
//                                 startIcon={reassigning ? <CircularProgress size={15} /> : <SyncAltOutlinedIcon sx={{ fontSize: 16 }} />}
//                                 sx={{
//                                     textTransform: "none", fontWeight: 700, borderRadius: "10px", px: 2.2, fontSize: 13.5,
//                                     borderColor: "#7c3aed", color: "#7c3aed",
//                                     "&:hover": { borderColor: "#5b21b6", bgcolor: "#f3e8ff" },
//                                 }}>
...
//                 <Button variant="contained" onClick={handleSave} disabled={saving || reassigning}
...
//     const openCreate = () => { setEditId(null); setEditTaskId(null); setEditForm(null); setDialogOpen(true); };

//     // ── Fixed: fully auto-populates the form with the selected task's current details ──
//     const openEdit = (row) => {
//         setEditId(row.id);
//         setEditTaskId(row.task_id || "");
//         const owners = Array.isArray(row.owner) ? row.owner : (row.owner ? [row.owner] : []);
//         // Owner field now supports a single selection only — keep the first owner if legacy data has more than one
//         const recipients = owners.length > 0 ? [{ name: owners[0], email: owners[0] }] : [];
//         setEditForm({
//             // `task` must be populated (not just taskInput) so the Autocomplete actually shows the current task
//             task: row.task ? { task: row.task, oem: row.oem ?? "", id: row.id } : null,
//             taskInput: row.task ?? "",
...
//     // Added "Reassign" column, sourced from `reassign_count`
//     const TABLE_HEADERS = ["SN", "Task ID", "Task", "Owner(s)", "Assigned By", "OEM", "Priority", "Reminder", "Deadline", "Status", "Reassign", "Actions"];
...
//                                                 {/* REASSIGN — sourced from `reassign_count` field returned by the API */}
//                                                 <TableCell>
//                                                     <Tooltip title={`Reassigned ${row.reassign_count ?? 0} time${(row.reassign_count ?? 0) === 1 ? "" : "s"}`} arrow>
//                                                         <Chip
//                                                             icon={<SyncAltOutlinedIcon sx={{ fontSize: "13px !important", color: "#0d47a1 !important" }} />}
//                                                             label={row.reassign_count ?? 0}
//                                                             size="small"
//                                                             sx={{
//                                                                 bgcolor: alpha("#0d47a1", 0.08), color: "#0d47a1",
//                                                                 fontWeight: 700, fontSize: 11, minWidth: 40,
//                                                                 border: `1px solid ${alpha("#0d47a1", 0.25)}`,
//                                                             }}
//                                                         />
//                                                     </Tooltip>
//                                                 </TableCell>
...
const AssignTaskDialog = ({ open, onClose, editId, taskId, initialForm, onSaved, taskOptionsList, userEmail, userID }) => {
    ...
    // ── Reassign Task: POST /assign_task/reassign/<pk>/ ────────────────────────
    // NOTE: Reassign is only offered for tasks that are already "Completed" (button is
    // conditionally rendered in the footer — see `canReassign`). Remarks are now compulsory
    // here — they must capture *why* the task is being reassigned.
    const handleReassign = async () => {
        if (!editId) return;
        if (!validate()) return;

        // ── Remarks required for reassignment ──
        if (!form.remarks || !form.remarks.trim()) {
            setErrors(e => ({ ...e, remarks: "Please explain why this task is being reassigned" }));
            return;
        }
    ...
    // ── Reassign button is only relevant once a task has reached "Completed" status ──
    const canReassign = editId && form.status === "Completed";
        ...
        <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth disableScrollLock
            ...
            <DialogContent sx={{ px: 3, pt: 2.5, pb: 1.5, overflowY: "auto" }}>
                <Box display="flex" flexDirection="column" gap={2.2}>
                    ...
                    {/* ── Remarks ─────────────────────────────────────────────────────── */}
                    {/* Remarks becomes compulsory when reassigning a completed task — see helperText/error below */}
                    <TextField label={canReassign ? "Remarks (required to reassign)" : "Remarks (optional)"}
                        value={form.remarks}
                        onChange={(e) => setField("remarks", e.target.value)}
                        placeholder={canReassign ? "Explain why this task is being reassigned…" : "Add any notes or instructions…"}
                        error={!!errors.remarks}
                        helperText={errors.remarks || (canReassign ? "Required if you click \"Reassign Task\"" : "")}
                        size="small" fullWidth multiline rows={2} sx={fieldSx} />
                </Box>
            </DialogContent>
            <DialogActions sx={{ px: 3, pb: 2.5, pt: 1.5, gap: 1 }}>
                ...
                {/* ── Reassign Task button — only shown once the task is "Completed" ── */}
                {canReassign && (
                    <Tooltip title="Send this task's current details to a new owner via the reassign API" arrow>
                        <span>
                            <Button variant="outlined" onClick={handleReassign} disabled={reassigning || saving}
