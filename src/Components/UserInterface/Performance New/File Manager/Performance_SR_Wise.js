...
// // ── Main Component ────────────────────────────────────────────────────────────
// // `onDateRangeChange(startDate, endDate)` is an optional callback fired whenever
// // the effective date range changes here, so a parent (e.g. Performance_SR_Wise_Main)
// // can capture it and hand the same range to the AT Ageing Summary dashboard —
// // keeping the two dashboards in sync.
// const Performance_SR_Wise = ({ onDateRangeChange } = {}) => {
//     const { loading, action } = useLoadingDialog();
//     const navigate = useNavigate();
...
//     // ── Let the parent know whenever the selected range changes, so other
//     // dashboards (e.g. AT Ageing Summary) can mirror the same range ────────
//     useEffect(() => {
//         if (typeof onDateRangeChange === "function" && effectiveStartDate && effectiveEndDate) {
...
const Performance_SR_Wise = ({ onDateRangeChange } = {}) => {
    ...
    // Guards against duplicate/overlapping requests (e.g. StrictMode double
    // effect-invoke, or a new param change firing before the old call settles)
    ...
    // ── Effective dates used for display / callback (derived) ──────────────
    //    In month mode these are the 26th(prev)->25th(selected) window,
    //    purely for local display/sync before the API responds. The API
    //    itself is driven by the `month` param and returns the authoritative
    //    range in `date_range`.
    ...
    const fetchData = async () => {
        ...
        // Cancel any request that's still in flight from a previous param change
        ...
        // Stale-response guard: only the latest request is allowed to update state
        ...
        try {
            ...
            if (thisRequestId !== requestIdRef.current) return; // superseded/aborted, ignore

            if (err?.code === "ERR_CANCELED" || err?.name === "CanceledError" || err?.name === "AbortError") {
                // Expected when a newer request supersedes this one — not a real error
                return;
            }
    ...
    useEffect(() => {
        ...
        return () => {
            clearTimeout(timer);
            // Cancel in-flight request if params change again before it resolves
    ...
    const dateRangeLabel = apiResponse?.date_range
        ? apiResponse.date_range
        : (effectiveStartDate && effectiveEndDate
            ? `${effectiveStartDate}  to  ${effectiveEndDate}`
            : "");
    const titleLabel   = dateRangeLabel ? `AT Report  |  ${dateRangeLabel}` : "AT Report";
