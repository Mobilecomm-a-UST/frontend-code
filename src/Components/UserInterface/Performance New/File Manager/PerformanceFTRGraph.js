...
//               onReset={resetMonthRange}
//             />
//             {/* ✅ FIX: circle filter always shown (was hidden for non-FTR/All filters) */}
//             <CircleMultiFilter circles={allCircleNames} selectedCircles={selectedCircles} onChange={setSelectedCircles} />
//           </Box>
...
//   // In trend-only mode we don't need per-circle bar columns at all —
//   // just month + Overall — so the line renders cleanly with nothing else on it.
//   const chartData = useMemo(() => {
//     if (!circleGraphData || !circleGraphData[tech]) return [];
//     const circleSeries = circleGraphData[tech].series || [];
//     const grandSeries = (grandGraphData && grandGraphData[tech]?.series) || [];
...
//     let fromKey = monthNameToSortKey(monthRange.from);
//     let toKey = monthNameToSortKey(monthRange.to);
//     if (fromKey == null && toKey == null) return chartData;
//     if (fromKey != null && toKey != null && fromKey > toKey) {
//       [fromKey, toKey] = [toKey, fromKey];
//     }
...
//             </Typography>
//             <Typography fontSize={13} color="text.secondary" mt={0.2}>
//               {isTrendOnly ? "Overall (grand total) monthly trend" : "Circles as bars per month · Overall (grand total) as trend line"}
//             </Typography>
//           </Box>
//           <MuiTooltip title="Refresh data" arrow>
//             <span>
...
//                       {/* <YAxis yAxisId="left" domain={[0, 100]} ticks={PERCENT_AXIS_TICKS} tick={{ fontSize: 11, fill: "#90a4ae" }} axisLine={false} tickLine={false} width={42} />
//                       <YAxis yAxisId="right" orientation="right" domain={[0, 100]} ticks={PERCENT_AXIS_TICKS} tick={{ fontSize: 11, fill: OVERALL_LINE_COLOR }} axisLine={false} tickLine={false} tickFormatter={(v) => `${v}%`} width={46} /> */}
//                       <Tooltip content={<CustomTooltip />} cursor={isTrendOnly ? false : { fill: "rgba(100,120,150,0.05)", radius: 6 }} />
//                       <Legend
//                         wrapperStyle={{ paddingTop: 20, paddingBottom: 4 }}
//                         iconSize={11}
//                         formatter={(value) => {
...
// ✅ NEW: parse "Mon YYYY" into { year, monthIdx }
const parseMonthName = (name) => {
  if (!name) return null;
  const [abbr, year] = name.split(" ");
  const idx = MONTH_ABBR.indexOf(abbr);
  const yr = parseInt(year, 10);
  if (idx === -1 || Number.isNaN(yr)) return null;
  return { year: yr, monthIdx: idx };
};

// ✅ NEW: convert the From/To month-range into concrete start_date / end_date
// strings ("YYYY-MM-DD") so the backend can compute the FTR data live for
// whatever range is selected — the same pattern FTR_Aging uses, which is why
// that dashboard shows the current month and this chart didn't.
// const getDateRangeForMonths = (fromName, toName) => {
//   const from = parseMonthName(fromName);
//   const to = parseMonthName(toName || fromName) || from;
//   if (!from || !to) return null;

//   let f = from;
//   let t = to;
//   const fKey = f.year * 12 + f.monthIdx;
//   const tKey = t.year * 12 + t.monthIdx;
//   if (fKey > tKey) {
//     [f, t] = [t, f];
//   }

//   const start_date = `${f.year}-${String(f.monthIdx + 1).padStart(2, "0")}-01`;
//   const lastDay = new Date(t.year, t.monthIdx + 1, 0).getDate();
//   const end_date = `${t.year}-${String(t.monthIdx + 1).padStart(2, "0")}-${String(lastDay).padStart(2, "0")}`;

//   return { start_date, end_date };
// };

const buildMonthParam = (fromName, toName) => {
  const from = parseMonthName(fromName);
  const to = parseMonthName(toName || fromName) || from;
  if (!from || !to) return null;
 
  let f = fromName;
  let t = toName || fromName;
...
function PerformanceFTRGraph() {
  ...
  // ✅ FIX: fetchAll now sends start_date/end_date derived from the current
  // monthRange (same approach FTR_Aging uses), so the backend computes data
  // live for the selected range instead of relying on a pre-aggregated
  // monthly table that may not yet include the current month.
  // const fetchAll = React.useCallback((range) => {
  //   const myRequestId = ++requestIdRef.current;
  //   setLoading(true);
  //   setError(null);

  //   const dateRange = getDateRangeForMonths(range?.from, range?.to);
  ...
  // on mount — mirrors FTR_Aging's useEffect + setTimeout pattern so selecting
  // "Jul 2026" actually asks the backend for July instead of only filtering
  // whatever was cached at page load.
  useEffect(() => {
  ...
  // Server now already scopes data to the requested start_date/end_date, but
  // this client-side filter is kept as a harmless safety net in case the
  // backend ever returns a slightly wider range than requested.
  const filteredChartData = useMemo(() => {
    if (!chartData.length) return chartData;
    let fromKey = monthNameToSortKey(monthRange.from);
    let toKey = monthNameToSortKey(monthRange.to);
