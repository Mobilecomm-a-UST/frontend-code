...
const UploadPerformanceAt = () => {
  const [performance, setPerformance] = useState({ filename: "", bytes: "" })
  const [pdate, setPdate] = useState()
  const [fileData, setFileData] = useState()
  const [count, setCunt] = useState()
  const [show, setShow] = useState(false)
  const navigate = useNavigate();
  const [open, setOpen] = useState(false);
  const fileLength = performance.filename.length
  ...
  const handleperformance = (event) => {
    ...
    setCunt(files.length)

    for (var i = 0; i < files.length; i++) {
      console.log('multipul file :', files[i].name)
    }
      ...
      <div>
        <div style={{ margin: 10, marginLeft: 20 }}>
          <Breadcrumbs aria-label="breadcrumb" itemsBeforeCollapse={2} maxItems={3} separator={<KeyboardArrowRightIcon fontSize="small" />}>
            ...
            <Typography color='text.primary'>Upload Report</Typography>
          </Breadcrumbs>
        </div>
        <Box className={classes.main_Box}>
          <Box className={classes.Back_Box} sx={{ width: { md: '75%', xs: '100%' } }}>
            <Box className={classes.Box_Hading} >
              UPLOAD PERFORMANCE AT REPORT
            </Box>
            <Stack spacing={2} sx={{ marginTop: "-40px" }} direction={'column'}>
              <Box className={classes.Front_Box} >
                <div className={classes.Front_Box_Hading}>
                  Select Performance AT File:-<span style={{ fontFamily: 'Poppins', color: "gray", marginLeft: 20 }}>{performance.filename}</span>
                </div>
                <div className={classes.Front_Box_Select_Button} >
                  <div style={{ float: "left" }}>
                    <Button variant="contained" component="label" color={performance.state ? "warning" : "primary"}>
                      select file
                      <input type="file" required onChange={handleperformance} hidden multiple />
                    </Button>
                  </div>
...
export default UploadPerformanceAt