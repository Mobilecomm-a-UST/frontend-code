...
export default function Analytics() {
    ...
    const [gpFilter, setGpFilter] = useState(25);
    ...
    const avgRevenue = revenueFilter !== "" ? Number(revenueFilter): tableData.length > 0 ? Math.round(tableData.reduce((sum, item) => sum + item.revenue, 0) /tableData.length): 0;
    ...
    const scatterData = tableData.map((item) => {
        ...
        // Q1
        if (item.revenue >= avgRevenue && item.gpPercent >= gpFilter)
            color = "#4CAF50";

        // Q2
        else if (item.revenue < avgRevenue && item.gpPercent >= gpFilter)
            color = "#2196F3";
        ...
        return {
            ...
            gp: item.gp,
            salary:item.resource,
            vendor:item.vendor,
            expense:item.expense,
            fixed:item.fixed,
            circle:item.circle,
            project:item.costCenter,
            revenue:item.revenue,
    ...
    return (
        <>
        <div style={{ padding: 25, background: "#f4f7fb", minHeight: "100vh" }} >
                    ...
                    <>
                        ...
                        >
                            ...
                            >
                                ...
                                >
                                    Revenue Threshold : ₹ {formatAmount(avgRevenue)} | GP Threshold : {gpFilter}%
                                </div>
                                <div
                                    style={{
                                        display: "flex",
                                ...
                                >
                                    ...
                                    <select
                                        value={gpFilter}
                                        onChange={(e) => setGpFilter(e.target.value === "" ? "" : Number(e.target.value))}
                                        style={{
                                            padding: "8px 14px",
                                            borderRadius: 8,
                                            border: "1px solid #ddd",
