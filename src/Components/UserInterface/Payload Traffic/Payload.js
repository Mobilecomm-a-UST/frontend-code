...
//     // Normalize every incoming role to lowercase (and trim stray spaces)
//     // so role checks below are case-insensitive regardless of how the
//     // backend/localStorage sends them (e.g. "Admin", "ADMIN", "admin").
//     const userType = (getDecreyptedData("user_type")?.split(',') || []).map(
...
//                                                     <Nav.Item eventKey="1-1" placement="rightStart" className="single-item-custom" onClick={() => { navigate('Upload_4G_Payload'); show(); setMenuButton(true) }}>
//                                                         Upload 4G Payload
//                                                     </Nav.Item>
//                                                     <Nav.Item eventKey="1-2" placement="rightStart" className="single-item-custom" onClick={() => { navigate('Upload_5G_Payload'); show(); setMenuButton(true) }}>
//                                                         Upload 5G Payload
//                                                     </Nav.Item>
//                                                 </Nav.Menu>
//                                                 <Nav.Item eventKey="2" placement="rightStart" icon={<SearchIcon />} className="single-item-custom" onClick={() => { navigate('Query_Traffic'); show(); setMenuButton(true) }}>
//                                                     Query Traffic
//                                                 </Nav.Item>
//                                                 <Nav.Item eventKey="3" placement="rightStart" icon={<HistoryIcon />} className="single-item-custom" onClick={() => { navigate('Upload_History'); show(); setMenuButton(true) }}>
//                                                     Upload History
//                                                 </Nav.Item>
//                                             </Nav>}
...
const Payload = () => {
    ...
    console.log("userType before ",getDecreyptedData("user_type"))
    const userType = (getDecreyptedData("user_type")?.split(',') || []).map(
        (role) => role?.trim().toLowerCase()
    )
    console.log("userType after ",userType)

    console.log("userType", (['admin', 'PTS_admin'].some(role => userType?.includes(role.toLowerCase()))))
                    ...
                    <Grid item xs={0} md={2} sx={{}}>
                        ...
                        <Box sx={{ display: { xs: 'none', md: 'inherit' } }} >
                            <Box sx={{ position: 'fixed', width: '16.5%' }} >
                                <Sidenav expanded={expanded} defaultOpenKeys={[]} appearance="subtle" style={{ minHeight: "670px", height: "100vh", backgroundColor: "#006e74", marginTop: 20, borderRadius: 10 }}>
                                    <Sidenav.Body>
                                        {(['admin', 'PTS_admin'].some(role => userType?.includes(role.toLowerCase()))) &&
