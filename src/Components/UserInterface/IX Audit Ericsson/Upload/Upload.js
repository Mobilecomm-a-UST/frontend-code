...
const Upload = () => {
    ...
    const handleSubmit = async () => {
        if (make4GFiles.filename.length > 0) {
            ...
            formData.append(`log_file`, make4GFiles.bytes);
            const response = await postData('ix_tracker_vi/sa-slicing-audit/', formData)
        ...
        <>
            <div style={{ margin: 5, marginLeft: 10 }}>
                <Breadcrumbs aria-label="breadcrumb" itemsBeforeCollapse={2} maxItems={3} separator={<KeyboardArrowRightIcon fontSize="small" />}>
                    ...
                    {/* <Link underline='hover' onClick={() => { navigate('/tools/ix_tools') }}>Audit Ericsson</Link> */}
                    <Typography color='text.primary'>Ericsson Upload</Typography>
                </Breadcrumbs>
            </div>
            <Slide
                direction='left'
                ...
                <Box>
                            ...
                            <Stack spacing={2} sx={{ marginTop: "-40px" }} direction={'column'}>
                                ...
                                <Box className={classes.Front_Box} >
                                    <div className={classes.Front_Box_Hading}>
                                        Select Text File:-<span style={{ fontFamily: 'Poppins', color: "gray", marginLeft: 20 }}>{make4GFiles.filename}</span>
                                    </div>
                    ...
                    <Box sx={{ textAlign: 'center', display: download ? 'block' : 'none' }}>
                        ...
                        >
                            ...
                            >
                                Download Audit Ericsson Report
                            </span>
                        </Button>
                    </Box>
                </Box>
