...
// //
// // IMPORTANT: the response does not carry a plan_id per group. We pair each
// // returned group with a plan id by ARRAY POSITION — i.e. we assume the
// // backend returns one group per requested plan id, in the same order they
// // were sent in `planIdsCsv`. This is what lets two plan ids that share the
// // same circle still get their own independent IP list/selection. If the
// // backend ever de-duplicates by circle (returning fewer groups than plan
// // ids), this pairing will be wrong — confirm the ordering contract with the
// // backend if IPs ever look mismatched.
...
// // /mwCeragon/get_serverip/ and lists IPs as selectable rows — one card PER
// // PLAN ID (not per circle), so two plan ids that happen to share a circle
// // each get their own independent IP choice.
// //
// // Selections are tracked PER PLAN ID as a single IP string (radio-style, not
// // multi-select), keyed by the full plan id string — matching the backend's
// // `selected_ips` contract:
// //   { "MW-N-UPE-22052026-4906": "10.55.88.131",
// //     "MW-N-MAH-19022026-806":  "10.55.88.137" }
// const IpSelectPanel = ({ selectedPlans, selectedIpByPlan, onSelectedIpsChange }) => {
//     const [planGroups, setPlanGroups] = useState([]); // [{ planId, circle, ip_list }]
...
//             .then((groups) => {
//                 if (cancelled) return;
//                 // Pair each returned group with a plan id by position — see
//                 // the note above fetchServerIpsForPlans for why.
//                 const paired = groups.map((group, idx) => ({
//                     ...group,
//                     planId: selectedPlans[idx] ?? group.circle,
//                 }));
//                 setPlanGroups(paired);
//             })
...
//     // Drop any selected IP belonging to a plan id that's no longer selected
//     // (e.g. the chip was removed from the search box).
//     useEffect(() => {
//         const validPlanIds = new Set(selectedPlans);
//         const next = {};
//         let changed = false;
//         Object.entries(selectedIpByPlan).forEach(([planId, ip]) => {
//             if (validPlanIds.has(planId)) next[planId] = ip;
//             else changed = true;
//         });
...
//     // Picking an IP replaces any previous pick for that plan id (radio
//     // behavior). Clicking the already-selected IP deselects it.
//     const pickIp = (planId, ip) => {
//         const next = { ...selectedIpByPlan };
//         if (next[planId] === ip) delete next[planId];
//         else next[planId] = ip;
//         onSelectedIpsChange(next);
//     };

//     const removePlanSelection = (planId) => {
//         const next = { ...selectedIpByPlan };
//         delete next[planId];
//         onSelectedIpsChange(next);
//     };
...
//     const selectedEntries = Object.entries(selectedIpByPlan); // [planId, ip][]
...
//                         (auto-suggested per selected Plan ID)
//                     </Typography>
//                 </Box>
//                 {totalSelected > 0 && (
//                     <Chip
...
//                         const pickedIp = selectedIpByPlan[group.planId];
//                         return (
//                             <Grid item xs={12} sm={6} md={4} key={group.planId}>
//                                 <Box
//                                     sx={{
...
//                                     }}
//                                 >
//                                     <Box display="flex" alignItems="flex-start" justifyContent="space-between" mb={1.2} gap={1}>
//                                         <Box display="flex" flexDirection="column" gap={0.4} sx={{ minWidth: 0 }}>
//                                             <Tooltip title={group.planId} arrow>
//                                                 <Chip
//                                                     label={group.planId}
//                                                     size="small"
//                                                     sx={{ bgcolor: TEAL_DARK, color: "#fff", fontWeight: 700, fontSize: 11, maxWidth: "100%" }}
//                                                 />
//                                             </Tooltip>
//                                             {group.circle && (
//                                                 <Typography fontSize={11} color="gray" fontWeight={600}>
//                                                     Circle: {group.circle}
//                                                 </Typography>
//                                             )}
//                                         </Box>
//                                         {pickedIp && (
//                                             <Typography fontSize={11} fontWeight={700} color={TEAL_DARK} sx={{ flexShrink: 0 }}>
//                                                 selected
//                                             </Typography>
//                                         )}
//                                     </Box>
...
//                                                     <Box
//                                                         key={ip}
//                                                         onClick={() => pickIp(group.planId, ip)}
//                                                         sx={{
//                                                             display: "flex",
//                                                             alignItems: "center",
//                                                             gap: 1,
...
//                                                         }}
//                                                     >
//                                                         {/* radio-style indicator: one pick per plan id */}
//                                                         <Box
//                                                             sx={{
...
//                         Selected:
//                     </Typography>
//                     {selectedEntries.map(([planId, ip]) => (
//                         <Chip
//                             key={planId}
//                             label={`${planId}: ${ip}`}
//                             onDelete={() => removePlanSelection(planId)}
...
// const LinkBudgetRow = ({ label, files, onUpload, onDelete, showError, canManage }) => (
//     <Box>
...
//     // ── ONE IP selected per PLAN ID, keyed by the full plan id string:
//     //    { "MW-N-UPE-22052026-4906": "10.55.88.131",
//     //      "MW-N-MAH-19022026-806":  "10.55.88.137" }
//     //    Keying by plan id (instead of circle) lets two plan ids that share
//     //    the same circle still get independent IP selections. ──
//     const [selectedIpByPlan, setSelectedIpByPlan] = useState({});
...
//     // ── link budget file 2 (traffic shifting) ─────────────────────────────────────
//     // linkFiles2: [{ file_name, download_url }]
...
//         // Backend expects a JSON dict keyed by PLAN ID, one IP each:
//         // {"MW-N-UPE-22052026-4906": "10.55.88.131", "MW-N-MAH-19022026-806": "10.55.88.137"}
//         formData.append("selected_ips", JSON.stringify(selectedIpByPlan));
...
//                                 {/* ── SERVER IP (auto-suggested via POST /get_serverip/, one card per Plan ID) ── */}
//                                 <IpSelectPanel
//                                     selectedPlans={selectedPlans}
//                                     selectedIpByPlan={selectedIpByPlan}
//                                     onSelectedIpsChange={setSelectedIpByPlan}
//                                 />

//                                 {/* ── LINK BUDGET FILE 1 ──
//                                      Section always renders (so 'microwave' users can still see the
//                                      file name and download it) — only Select File / Delete are
//                                      hidden for them, via the canManage prop below. */}
//                                 <Box className={classes.Front_Box}>
//                                     <LinkBudgetRow
...
//                                         canManage={!userType.includes('microwave')}
//                                     />
//                                 </Box>
...
// ─── Multi-select Dump File Box ──────────────────────────────────────────────
// CHANGED: now lists every selected file individually with its own ✕ delete
// button (onRemoveFile), plus a "Clear all" action (onClearAll) when there's
// more than one file. Files still accumulate across multiple "Select File"
// clicks — this only adds granular control over what's currently selected.
const DumpFileBox = ({ label, files, onAddFiles, onRemoveFile, onClearAll }) => (
    <Box sx={{
        border: "1.5px solid #e0e0e0", borderRadius: "10px",
        p: 1.4, bgcolor: "#fafbfc", height: "100%",
        display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center",
        ...
        {files.length > 0 ? (
            <Box sx={{ width: "100%", mt: 1 }}>
                {/* per-file list, each row deletable on its own */}
                <Box
                    sx={{
                        width: "100%",
                        maxHeight: 140,
                ...
                >
                    {files.map((file, index) => {
                        const key = `${file.name}_${file.size}_${file.lastModified}_${index}`;
                        return (
                            <Box
                                key={key}
                                sx={{
                ...
                <Box display="flex" alignItems="center" justifyContent="center" gap={0.7} mt={0.9}>
                    <Typography color="green" fontSize={12} fontWeight={600}>
                        {files.length} file(s) selected
                    </Typography>
                    {files.length > 1 && (
...
const MicrowaveCeragonUpload = () => {
    ...
    // ── dump file handlers ───────────────────────────────────────────────────────
    // Adds newly selected files, skipping exact duplicates (same name/size/lastModified).
    const addFiles = (event, setFileState, errorKey) => {
        const selected = Array.from(event.target.files || []);
        if (selected.length === 0) return;
        setShowError((prev) => ({ ...prev, [errorKey]: false }));
    ...
    // NEW: removes a single file from a dump list by its index.
    const removeFile = (setFileState, index) => {
                                    ...
                                    <Grid container spacing={1.5}>
                                        <Grid item xs={6}>
                                            <DumpFileBox
                                                ...
                                                files={report_File}
                                                onAddFiles={(e) => addFiles(e, setReport_File, "report")}
                                                onRemoveFile={(index) => removeFile(setReport_File, index)}
                                                onClearAll={() => setReport_File([])}
                                            />
                                        </Grid>
                                        <Grid item xs={6}>
                                            <DumpFileBox
                                                ...
                                                files={report_File2}
                                                onAddFiles={(e) => addFiles(e, setReport_File2, "report")}
                                                onRemoveFile={(index) => removeFile(setReport_File2, index)}
                                                onClearAll={() => setReport_File2([])}
                                            />
                                        </Grid>
                                    </Grid>
