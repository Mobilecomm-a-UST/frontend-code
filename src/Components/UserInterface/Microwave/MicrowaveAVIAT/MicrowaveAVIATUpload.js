...
const MicrowaveAVIATUpload = () => {
  ...
  const [report_File, setReport_File] = useState({ filename: "", bytes: "" });
  const [radio_File, setRadio_File] = useState({ filename: "", bytes: "" });
  const [config_File, setConfig_File] = useState({ filename: "", bytes: "" });
  ...
  const handleSubmit = async () => {
    ...
    if (!isValid) {
      setShowError({
        ...
        report: !report_File.bytes.length,
        radio: !radio_File.bytes.length,
        config: !config_File.bytes.length,
      });
      return;
    }
    ...
    for (let m = 0; m < config_File.bytes.length; m++) {
      formData.append("config_file", config_File.bytes[m]);
    }
  ...
  const handleCancel = () => {
    setReport_File({ filename: "", bytes: "" });
    setRadio_File({ filename: "", bytes: "" });
    setConfig_File({ filename: "", bytes: "" });
    setDownload(false);
    setShowError({ budget: false, report: false, radio: false, config: false });
  };
            ...
            <Box className={classes.Back_Box} sx={{ width: { md: "75%", xs: "100%" } }}>
              ...
              <Stack spacing={2} sx={{ mt: "-40px" }}>
                ...
                <UploadSection
                  ...
                  color={config_File.filename ? "warning" : "primary"}
                  onChange={(e) => updateFile(e, setConfig_File, "config")}
                  error={showError.config}
                  selectedText={config_File.filename}
                />
              </Stack>
