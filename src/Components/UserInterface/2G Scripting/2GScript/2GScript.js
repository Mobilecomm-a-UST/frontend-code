...
const Script2G = () => {
  ...
  const handleSubmit = async () => {
    if (make2GFiles.length > 0 && selectCircle !== '') {
      action(true)
      ...
      for (let i = 0; i < make2GFiles.length; i++) {
        ...
        formData.append(`file`, make2GFiles[i]);
      }

      const response = await postData('noika2G/script/', formData)
  ...
  return (
    ...
    <div style={{ margin: 5, marginLeft: 10 }}>
        <Breadcrumbs aria-label="breadcrumb" itemsBeforeCollapse={2} maxItems={3} separator={<KeyboardArrowRightIcon fontSize="small" />}>
          ...
          <Link underline="hover" onClick={() => { navigate('/tools/2g_scripting') }}>2G Scripting</Link>
          <Typography color='text.primary'>2G Script</Typography>
        </Breadcrumbs>
      </div>
      <Slide
        direction='left'
      ...
      >
        <Box>
                ...
                <Box className={classes.Front_Box} >
                  ...
                  <div className={classes.Front_Box_Select_Button} >
                    <div style={{ float: "left" }}>
                      <Button variant="contained" component="label" color={make2GFiles.length > 0 ? "warning" : "primary"}>
                        select file
                    ...
                    {make2GFiles.length > 0 && <span style={{ color: 'green', fontSize: '18px', fontWeight: 600 }}>Selected File(s) : {make2GFiles.length}</span>}

                    <div>  <span style={{ display: show2G ? 'inherit' : 'none', color: 'red', fontSize: '18px', fontWeight: 600 }}>This Field Is Required !</span> </div>
                  </div>
                </Box>
              </Stack>
          ...
          <Box sx={{ display: download ? 'block' : 'none', textAlign: 'center' }}>
            <a download href={fileData}><Button variant="outlined" onClick='' startIcon={<FileDownloadIcon style={{ fontSize: 30, color: "green" }} />} sx={{ marginTop: "10px", width: "auto" }}><span style={{ fontFamily: "Poppins", fontSize: "22px", fontWeight: 800, textTransform: "none", textDecorationLine: "none" }}>Download 2G Script</span></Button></a>
          </Box>
        </Box>
      </Slide>
      {loading}
