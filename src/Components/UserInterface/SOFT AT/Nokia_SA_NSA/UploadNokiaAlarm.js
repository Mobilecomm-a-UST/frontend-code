...
const UploadNokiaAlarm = () => {
    const [alarm, setAlarm] = useState({ filename: "", bytes: "" })
    ...
    const handleSubmit = async () => {
        if (make4GFiles.length > 0) {
            ...
            for (let i = 0; i < make4GFiles.length; i++) {
                ...
                formData.append(`alarm_file`, make4GFiles[i]);
            }
            formData.append("mapping_file", alarm.bytes);

            const response = await postData('daily_alarm/Nokia/', formData)
        ...
        <>
            <div style={{ margin: 5, marginLeft: 10 }}>
                <Breadcrumbs aria-label="breadcrumb" itemsBeforeCollapse={2} maxItems={3} separator={<KeyboardArrowRightIcon fontSize="small" />}>
                    ...
                    <Typography color='text.primary'>Upload Nokia Alarm</Typography>
                </Breadcrumbs>
            </div>
            <Slide
                direction='left'
                    ...
                    <Box className={classes.main_Box}>
                        <Box className={classes.Back_Box} sx={{ width: { md: '75%', xs: '100%' } }}>
                            <Box className={classes.Box_Hading} >
                                Upload Nokia Alarm Files
                            </Box>
                            <Stack spacing={2} sx={{ marginTop: "-40px" }} direction={'column'}>

                                <Box className={classes.Front_Box} >
                                    <div className={classes.Front_Box_Hading}>
                                        Select Alarm Files:-<span style={{ fontFamily: 'Poppins', color: "gray", marginLeft: 20 }}>{ }</span>
                                    </div>
                                    <div className={classes.Front_Box_Select_Button} >
                                        <div style={{ float: "left" }}>
                                            <Button variant="contained" component="label" color={make4GFiles.length > 0 ? "warning" : "primary"}>
                                                select file
                                                <input required hidden accept=".xlsx,.csv" multiple type="file"
                                ...
                                <Box className={classes.Front_Box} >
                                    <div className={classes.Front_Box_Hading}>
                                        Select Mapping File:-<span style={{ fontFamily: 'Poppins', color: "gray", marginLeft: 20 }}>{alarm.filename}</span>
                                    </div>
                                    <div className={classes.Front_Box_Select_Button} >
                                        <div style={{ float: "left" }}>
                                            <Button variant="contained" component="label" color={alarm.state ? "warning" : "primary"}>
                                                select file
                                                <input required onChange={handleAlarm} hidden accept=".xlsx,.csv" multiple type="file" />
                                            </Button>
                                        </div>
...
export default UploadNokiaAlarm;