...
const BasebandUpload = () => {
    ...
    const handleSubmit = async () => {
        if (make4GFiles.length > 0) {
            ...
            const response = await postData('Baseband/baseband_req/', formData)
        ...
        <>
            <div style={{ margin: 5, marginLeft: 10 }}>
                <Breadcrumbs aria-label="breadcrumb" itemsBeforeCollapse={2} maxItems={3} separator={<KeyboardArrowRightIcon fontSize="small" />}>
                    ...
                    <Link underline="hover" onClick={() => { navigate('/tools/baseband_requirement') }}>Baseband Requirement</Link>
                    <Typography color='text.primary'>Baseband Upload</Typography>
                </Breadcrumbs>
            </div>
            <Slide
                direction='left'
