...
const SleepingCell = () => {
    ...
    const fetchDashboardData = useCallback(async () => {
      ...
      formData.append('ticket_type', 'Sleeping Cell');
      try {
        const res = await makePostRequest("Zero_Count_Rna_Payload_Tool/circle_wise_open_close_dashboard/", formData);
        action(false);
  
        if (res) {
        //   console.log('sleeping cell data res', res)
          setFromDate(res.from_date);
          setToDate(res.to_date);
    ...
    const options = {
      ...
      plugins: {
        ...
        title: {
          display: true,
          text: `Sleeping Cell Status ( ${handleDateFormat(fromDate)} ~ ${handleDateFormat(toDate)} ) Total : ${handleTotalCount(openPayloadDip) + handleTotalCount(closePayloadDip)} ${priority.length > 0 ? '/ Priority : ' + priority : ''} ${agingBucket.length > 0 ? '/ Aging : ' + agingBucket : ''}`,
          font: {
            size: 16,
            weight: 'bold'
          }
