...
const DateWise = () => {
    ...
    const handleGapHiperlink = async (milestone1, milestone2, gap) => {
        ...
        if (gap == 0 || gap === '-') {
            ...
            if (milestone2 && milestone1 && gap) {
                ...
                try {
                    const res = await postData("nt_tracker/gap_view/", formData);
    ...
    const handleExportExcel = async () => {
        ...
        a.href = url;
        a.download = "NT_Daily_Progress-RFAI_to_MS1_Waterfall.xlsx";
        a.click();
    ...
    const ClickDataGet = async (props) => {
        ...
        const responce = await makePostRequest('nt_tracker/hyperlink_frontend_editing/', formData)
        if (responce) {
