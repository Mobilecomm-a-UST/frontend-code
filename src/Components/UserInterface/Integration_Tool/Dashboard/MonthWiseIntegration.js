...
const MonthWiseIntegration = ({ onData }) => {
    ...
    const ClickDataGet = async (props) => {
        ...
        const responce = await makePostRequest('IntegrationTracker/hyperlink-monthwise-integration-data/', formData)
        if (responce) {
                                ...
                                <thead style={{ position: 'sticky', top: 0, zIndex: 1 }}>
                                    <tr style={{ fontSize: 15, backgroundColor: "#223354", color: "white", border: '1px solid white' }}>
                                        ...
                                        {monthArray?.map((item, index) => index < 3 && (
                                            <th colSpan='16' key={index} style={{ padding: '5px 20px', whiteSpace: 'nowrap', backgroundColor: headerColor[index] }}>{monthNames[item.month]}-{item.year}</th>
                                        ))}
                                    </tr>
                                    <tr style={{ fontSize: 15, backgroundColor: "#223354", color: "white", border: '1px solid white' }}>
