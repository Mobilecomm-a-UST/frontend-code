...
const RangeWiseDashboard = ({ onData }) => {
    ...
    // ✅ Helper: returns style + onClick based on whether value is zero/empty
    const getCellProps = (value, clickParams) => {
        const isDisabled = !value || Number(value) === 0;
        return {
            style: {
                cursor: isDisabled ? 'default' : 'pointer',
                opacity: isDisabled ? 0.4 : 1,
                pointerEvents: isDisabled ? 'none' : 'auto',  // fully blocks click events
            },
            ...
            <Slide direction="left" in='true' timeout={700} style={{ transformOrigin: '1 1 1' }}>
                <div style={{ margin: 20 }}>
                    <Box style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', alignContent: 'center' }}>
                        ...
                        <Box>
                            {/* ✅ From Date — no future dates allowed */}
                            <TextField
                                size='small'
                                value={fromDate}
                                onChange={(e) => { setFromDate(e.target.value) }}
                            ...
                            {/* ✅ To Date — no future dates allowed */}
                            <TextField
                                size='small'
                                value={toDate}
                                onChange={(e) => { handleDate(e.target.value) }}
                                    ...
                                    {tableData?.map((it, index) => {
                                        return (
                                            <tr style={{ textAlign: "center", fontWeigth: 700 }} key={index}>
                                                ...
                                                {/* ✅ Each cell uses getCellProps — disabled when value is 0 */}
                                                ...
                                                <th {...getCellProps(it?.D1_OTHERS, { date: dateArray[2], circle: it?.cir, activity: 'OTHERS' })}>{it?.D1_OTHERS}</th>
                                                <th {...getCellProps(it?.D1_RELOCATION, { date: dateArray[2], circle: it?.cir, activity: 'RELOCATION' })}>{it?.D1_RELOCATION}</th>
                                                <th {...getCellProps(it?.D1_RET, { date: dateArray[2], circle: it?.cir, activity: 'RET' })}>{it?.D1_RET}</th>
                                                ...
                                                <th {...getCellProps(it?.D1_FEMTO, { date: dateArray[2], circle: it?.cir, activity: 'FEMTO' })}>{it?.D1_FEMTO}</th>
                                                <th {...getCellProps(it?.D1_HT_INCREMENT, { date: dateArray[2], circle: it?.cir, activity: 'HT INCREMENT' })}>{it?.D1_HT_INCREMENT}</th>
                                                <th {...getCellProps(it?.D1_IBS, { date: dateArray[2], circle: it?.cir, activity: 'IBS' })}>{it?.D1_IBS}</th>
                                                <th {...getCellProps(it?.D1_IDSC, { date: dateArray[2], circle: it?.cir, activity: 'IDSC' })}>{it?.D1_IDSC}</th>
                                                <th {...getCellProps(it?.D1_ODSC, { date: dateArray[2], circle: it?.cir, activity: 'ODSC' })}>{it?.D1_ODSC}</th>
                                                <th {...getCellProps(it?.D1_RECTIFICATION, { date: dateArray[2], circle: it?.cir, activity: 'RECTIFICATION' })}>{it?.D1_RECTIFICATION}</th>
                                                <th {...getCellProps(it?.D1_OPERATIONS, { date: dateArray[2], circle: it?.cir, activity: 'OPERATIONS' })}>{it?.D1_OPERATIONS}</th>
                                                <th {...getCellProps(it?.D1_RRU_UPGRADE, { date: dateArray[2], circle: it?.cir, activity: 'RRU UPGRADE' })}>{it?.D1_RRU_UPGRADE}</th>
                                                <th {...getCellProps(it?.D1_5G_BW_UPGRADE, { date: dateArray[2], circle: it?.cir, activity: '5G BW UPGRADE' })}>{it?.D1_5G_BW_UPGRADE}</th>
                                                <th {...getCellProps(it?.D1_5G_RRU_SWAP, { date: dateArray[2], circle: it?.cir, activity: '5G RRU SWAP' })}>{it?.D1_5G_RRU_SWAP}</th>
                                                <th {...getCellProps(it?.D1_5G_SECTOR_ADDITION, { date: dateArray[2], circle: it?.cir, activity: '5G SECTOR ADDITION' })}>{it?.D1_5G_SECTOR_ADDITION}</th>
                                                <th {...getCellProps(it?.D1_5G_RELOCATION, { date: dateArray[2], circle: it?.cir, activity: '5G RELOCATION' })}>{it?.D1_5G_RELOCATION}</th>
                                                <th {...getCellProps(it?.D1_TRAFFIC_SHIFTING, { date: dateArray[2], circle: it?.cir, activity: 'TRAFFIC SHIFTING' })}>{it?.D1_TRAFFIC_SHIFTING}</th>
                                                <th {...getCellProps(it?.D1_RRU_SWAP, { date: dateArray[2], circle: it?.cir, activity: 'RRU SWAP' })}>{it?.D1_RRU_SWAP}</th>
                                            </tr>
                                        )
                                    })}
                                    <tr style={{ textAlign: "center", fontWeigth: 700, backgroundColor: '#B0EBB4', color: '#000000', fontSize: 17 }}>
