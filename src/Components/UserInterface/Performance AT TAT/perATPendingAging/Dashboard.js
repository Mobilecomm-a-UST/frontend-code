...
//         console.log("Download URL:", apiResponse?.download_url);
...
const Dashboard = () => {
    ...
    // ✅ FIX 1: Wrapped in useCallback so fetchData has a stable reference
    // and correctly captures the latest activeType & selectedMonth values.
    const fetchData = useCallback(async () => {
        ...
        // ✅ FIX 2: Debug log — verify correct tab & month are being sent to API
        // console.log("Fetching →", { activeType, selectedMonth });

        action(true);

        const api = activeType === "performance"
            ? "performance_tat/generate-tat-report/"
            : "performance_tat/generate-offered/";

        const formData = new FormData();
        formData.append("month", selectedMonth);

        // ✅ FIX 3: Also send as JSON body in case backend reads JSON not FormData
        // Uncomment the block below and comment out FormData above if backend uses JSON:
        /*
        const res = await postData(api, { month: selectedMonth });
        */

        const res = await postData(api, formData);

        console.log(`API Response [${activeType}] [${selectedMonth}]:`, res);

        if (res?.status) setApiResponse(res);
        else setApiResponse(null);

        action(false);
    // ✅ Both activeType and selectedMonth are in deps — ensures fresh data on every change
    }, [activeType, selectedMonth]);

    // ✅ FIX 4: useEffect depends on fetchData (which itself tracks activeType + selectedMonth)
    // This replaces the old pattern that had a stale-closure risk.
    useEffect(() => {
        fetchData();
    }, [fetchData]);
    ...
    // ✅ FIX 5: Reset apiResponse when switching tabs so stale data isn't shown
