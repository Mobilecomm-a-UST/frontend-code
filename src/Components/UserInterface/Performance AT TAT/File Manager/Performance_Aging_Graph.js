                ...
                <Paper elevation={0} sx={{
                    ...
                    <Box>
                        <Typography fontWeight={800} fontSize={20} letterSpacing="-.3px" color="#1a1a2e">
                            Performance Aging Graph - [Overall as a Trend]
                        </Typography>
...
// // ── Circle / bar colour palette (one distinct colour per circle) ──────────────
// // These match Excel-like distinct colours: blue, orange, green, cyan, purple…
// const CIRCLE_BAR_COLORS = [
//     "#1565c0",  // deep blue   → AP
//     "#e65100",  // deep orange → TN
//     "#2e7d32",  // dark green  → JK
//     "#0097a7",  // teal/cyan   → HR
//     "#6a1b9a",  // purple
//     "#c62828",  // red
//     "#4e342e",  // brown
...
// // ─────────────────────────────────────────────────────────────────────────────
// // DATA TRANSFORM  (REDESIGNED to match Excel layout)
// // ─────────────────────────────────────────────────────────────────────────────
// // Excel layout:
// //   X-axis  = months  (Jan, Feb, Mar …)
// //   Bars    = one bar series per circle  (AP, TN, JK, HR …)
// //   Line    = "Overall" value per month
// //
// // API shape expected (per-month response):
// //   res.graph_data[tech].series  = [
// //     { name: "Jun 2026", circles: { AP: [v0,v1,v2,v3,v4], TN: [...], ... }, overall: 32 }
// //   ]
// //   where v0…v4 map to categories <12%, <13-21%, <22-30%, >30days%, Pending%
// //
// // After mergeResponses one entry per month is in series[].
// //
// // transformToMonthView builds rows like:
// //   [
// //     { month: "Jan 2026", AP: 12, TN: 34, JK: 32, HR: 21, Overall: 32 },
// //     { month: "Feb 2026", AP: 13, TN: 35, JK: 33, HR: 22, Overall: 33 },
// //     …
// //   ]
// // The value used per circle is determined by the selected category filter:
// //   "ALL"     → sum (or total) across all 5 categories for that circle
// //   "<12%"    → category index 0
// //   "<13-21%" → category index 1
// //   "<22-30%" → category index 2
// //   ">30days%"→ category index 3
// //   "Pending%"→ category index 4
...
// const getCircleValue = (circleArr, selectedCategories) => {
//     if (!Array.isArray(circleArr)) return null;
//     const isAll = !selectedCategories.length || selectedCategories.includes("ALL");
//     if (isAll) {
//         // Sum all category values for this circle → "Total" across categories
//         const nums = circleArr.filter(v => typeof v === "number");
//         return nums.length ? Math.round(nums.reduce((a, b) => a + b, 0) * 10) / 10 : null;
//     }
//     // Single category selected → use its index value
//     const catKey = selectedCategories[0];
//     const idx = CATEGORY_INDEX_MAP[catKey];
//     return typeof circleArr[idx] === "number" ? circleArr[idx] : null;
// };

// // Merge multiple month API responses into one object
// const mergeResponses = (responses) => {
//     if (!responses?.length) return null;
//     const valid = responses.filter(Boolean);
//     if (!valid.length) return null;
...
//             if (td.series?.length) allSeries.push(...td.series);
//         });
//         // De-dup by series name (month label)
//         const seen = new Map();
//         allSeries.forEach((s) => seen.set(s.name, s));
//         merged.graph_data[tech] = { categories, series: Array.from(seen.values()) };
//     });
...
// // Build month-view chart rows from merged API data + active category filter
// const transformToMonthView = (techData, selectedCategories) => {
//     if (!techData?.series?.length) return { chartData: [], circleNames: [] };
...
//     // Collect all circle names across all months
//     series.forEach((s) => {
//         if (s.circles) Object.keys(s.circles).forEach(c => circleSet.add(c));
//     });
//     const circleNames = Array.from(circleSet).sort();
...
//     const chartData = series.map((s) => {
//         const row = { month: s.name };   // e.g. "Jun 2026"
//         circleNames.forEach((circle) => {
//             row[circle] = getCircleValue(s.circles?.[circle], selectedCategories);
//         });
//         // Overall line value
//         if (typeof s.overall === "number") {
//             row["Overall"] = s.overall;
//         } else if (typeof s.overall_pct === "number") {
//             row["Overall"] = s.overall_pct;
//         } else {
//             // Fallback: average of circle values
//             const vals = circleNames.map(c => row[c]).filter(v => typeof v === "number");
//             row["Overall"] = vals.length
...
//     monthLabel,
// }) => {
//     // Build month-view data fresh whenever category filter or source data changes
//     const { chartData, circleNames: allCircleNames } = useMemo(
//         () => transformToMonthView(techData, selectedCategories),
//         [techData, selectedCategories]
//     );

//     // Respect circle multi-filter: if specific circles are chosen, show only those
//     const circleNames = useMemo(() => {
//         if (!selectedCircles?.length) return allCircleNames;
//         return allCircleNames.filter(c => selectedCircles.includes(c));
//     }, [allCircleNames, selectedCircles]);
...
//                             {/* One bar per circle — each circle gets its own colour */}
//                             {circleNames.map((circleName, i) => (
//                                 <Bar
//                                     key={circleName}
//                                     yAxisId="left"
...
//                             {/* Overall line — single trend line across months, like Excel */}
//                             <Line
//                                 yAxisId="right"
//                                 type="monotone"
//                                 dataKey="Overall"
...
//     // Drop de-selected circles that disappeared from the current tech
//     useEffect(() => {
//         if (selectedCircles.length && circleOptions.length) {
//             const next = selectedCircles.filter((c) => circleOptions.includes(c));
//             if (next.length !== selectedCircles.length) setSelectedCircles(next);
...
//                         </Typography>
//                         <Typography fontSize={13} color="text.secondary" mt={0.2}>
//                             Circles as bars per month · Overall as trend line — matching Excel view
//                         </Typography>
//                     </Box>
//                     <Box display="flex" gap={1} alignItems="center" flexWrap="wrap">
//                         <MonthRangePicker
