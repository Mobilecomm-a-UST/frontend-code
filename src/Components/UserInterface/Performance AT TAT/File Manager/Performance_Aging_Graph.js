...
// // ─────────────────────────────────────────────────────────────────────────────
// // CUSTOM X-AXIS TICK — shows circle name
// // ─────────────────────────────────────────────────────────────────────────────
// const CustomXTick = ({ x, y, payload }) => (
...
// // ─────────────────────────────────────────────────────────────────────────────
// // CUSTOM Y-AXIS TICK — shows % suffix
// // ─────────────────────────────────────────────────────────────────────────────
// const CustomYTick = ({ x, y, payload }) => (
//     <text x={x} y={y + 4} textAnchor="end" fontSize={11} fill="#90a4ae">
//         {payload.value}%
//     </text>
// );

// // ─────────────────────────────────────────────────────────────────────────────
// // CIRCLE MULTI-SELECT DROPDOWN
// // ─────────────────────────────────────────────────────────────────────────────
// const CircleFilter = ({ allCircles, selectedCircles, onChange, themeColor }) => {
//     const isAllSelected = selectedCircles.length === 0 || selectedCircles.length === allCircles.length;
...
//             ? selectedCircles[0]
//             : `${selectedCircles.length} circles selected`;
...
//                             borderRadius: "10px",
//                             fontSize: 13,
//                             "& fieldset": { borderColor: alpha(themeColor, 0.4) },
...
//                 }
//                 renderValue={() => (
//                     <Typography fontSize={12.5} fontWeight={600} color={themeColor}>
//                         {displayLabel}
//                     </Typography>
//                 )}
//                 MenuProps={{
...
//             >
//                 {/* ALL option */}
//                 <MenuItem value="__ALL__" sx={{ borderBottom: "1px solid #f0f0f0", mb: 0.5 }}>
//                     <Checkbox
//                         checked={isAllSelected}
//                         indeterminate={!isAllSelected && selectedCircles.length > 0}
//                         size="small"
//                         sx={{ color: themeColor, "&.Mui-checked": { color: themeColor } }}
//                     />
//                     <ListItemText
//                         primary={
//                             <Typography fontSize={13} fontWeight={700} color={themeColor}>
//                                 All Circles
//                             </Typography>
//                         }
//                     />
//                     {isAllSelected && (
//                         <Chip label={allCircles.length} size="small"
//                             sx={{ bgcolor: themeColor, color: "#fff", fontWeight: 700, fontSize: 10, ml: 1 }} />
//                     )}
//                 </MenuItem>

//                 {allCircles.map((circle) => (
//                     <MenuItem key={circle} value={circle} sx={{ py: 0.6 }}>
//                         <Checkbox
//                             checked={selectedCircles.includes(circle)}
//                             size="small"
//                             sx={{ color: themeColor, "&.Mui-checked": { color: themeColor } }}
//                         />
//                         <ListItemText
//                             primary={
//                                 <Typography fontSize={13} fontWeight={500}>{circle}</Typography>
//                             }
//                         />
//                     </MenuItem>
//                 ))}
...
// }) => {
//     const isAllCatSelected = selectedCategories.length === 0 || selectedCategories.includes("ALL");

//     // ── Step 1: Filter by category ────────────────────────────────────────
//     const categoryFilteredData = useMemo(() => {
//         if (isAllCatSelected) return chartData;
//         return chartData.filter((row) =>
//             selectedCategories.some(
//                 (cat) => normalizeCategory(row.category) === normalizeCategory(cat)
//             )
//         );
//     }, [chartData, selectedCategories, isAllCatSelected]);

//     // ── Step 2: Pivot data so X-axis = circles, series = categories ───────
//     // Each row in chartData = { category: "AP", "Jun 2026": 29.6, ... }
//     // We want: { circle: "AP", "<12%": 29.6, "<13-21%": 34.5, ... }
//     //
//     // The API returns: categories = circle names, series = [{name: "Jun 2026", data: [...]}]
//     // But for circle-wise view, we need to pivot so circles are X-axis items
//     // and the category % columns ("<12%", "<13-21%" etc.) are series.
//     //
//     // Since API gives: categories=["<12%","<13-21%",...], series=[{name:"Jun 2026", data:[29.6,34.5,...]}]
//     // We need to pivot: X = circles from allCircles, Y = each category value for that circle
//     //
//     // For now, use the data as-is (category on X, months as series)
//     // and add circle filtering by filtering series names that match selected circles.

//     // ── Determine all available circles from series names ─────────────────
//     const allCircles = useMemo(() => {
//         return seriesNames; // series names ARE the circle names (months in single month, circles in circle-pivot)
//     }, [seriesNames]);

//     // ── Filter series (circles) based on selectedCircles ─────────────────
//     const filteredSeriesNames = useMemo(() => {
//         if (!selectedCircles || selectedCircles.length === 0) return seriesNames;
//         return seriesNames.filter((name) => selectedCircles.includes(name));
//     }, [seriesNames, selectedCircles]);

//     const hasData = categoryFilteredData.length > 0 && filteredSeriesNames.length > 0;

//     const numSeries     = filteredSeriesNames.length;
//     const chartHeight   = Math.max(420, 400 + Math.max(0, numSeries - 4) * 16);
//     const maxBarSize    = numSeries <= 2 ? 48 : numSeries <= 4 ? 36 : numSeries <= 6 ? 26 : 20;
//     const barCategoryGap = numSeries <= 3 ? "30%" : numSeries <= 6 ? "25%" : "20%";
//     const barGap         = numSeries <= 4 ? 4 : 3;
...
//                     {/* Show selected circles as chips in header */}
//                     {filteredSeriesNames.slice(0, 6).map((s, i) => {
//                         const g = SERIES_GRADIENTS[i % SERIES_GRADIENTS.length];
//                         return (
//                             <Chip
//                                 key={s}
...
//                         );
//                     })}
//                     {filteredSeriesNames.length > 6 && (
//                         <Chip
//                             label={`+${filteredSeriesNames.length - 6} more`}
//                             size="small"
//                             sx={{ background: alpha("#fff", 0.2), color: "#fff", fontWeight: 700, fontSize: 10 }}
//                         />
//                     )}
//                 </Box>
//             </Box>
...
//                         <Typography fontSize={14} fontWeight={600} color="text.secondary">No Data Available</Typography>
//                         <Typography fontSize={12} color="text.disabled">
//                             {!isAllCatSelected ? "No records for selected categories" : "Try adjusting the date range or circle filter"}
//                         </Typography>
//                     </Box>
//                 ) : (
//                     <ResponsiveContainer width="100%" height={chartHeight}>
...
//                             margin={{ top: 32, right: 60, left: 10, bottom: 8 }}
//                             barCategoryGap={barCategoryGap}
//                             barGap={barGap}
//                         >
//                             <GradientDefs />
...
//                                     const g = SERIES_GRADIENTS[index % SERIES_GRADIENTS.length];
//                                     return (
//                                         <span style={{ color: g.label, fontWeight: 700, fontSize: 11.5 }}>
//                                             {value}
//                                         </span>
//                                     );
//                                 }}
...
//                             {filteredSeriesNames.map((name, i) => {
//                                 const g = SERIES_GRADIENTS[i % SERIES_GRADIENTS.length];
//                                 return (
//                                     <Bar
//                                         key={name}
...
//                             {/* ✅ Reference lines for each category value (right-side % markers) */}
//                             {categoryFilteredData.map((row) => {
//                                 const maxVal = Math.max(
//                                     ...filteredSeriesNames.map((n) => row[n] ?? 0)
//                                 );
//                                 if (!maxVal) return null;
//                                 return (
//                                     <ReferenceLine
//                                         key={`ref-${row.category}`}
//                                         y={maxVal}
//                                         stroke={alpha("#546e7a", 0.25)}
//                                         strokeDasharray="4 4"
//                                         label={{
//                                             value: `${maxVal}%`,
//                                             position: "right",
...
//     const [activeTech,         setActiveTech]         = useState("4G");
//     const [selectedCategories, setSelectedCategories] = useState([]);
//     const [selectedCircles,    setSelectedCircles]    = useState([]);   // ✅ NEW
...
//     // ── Reset circle selection when data changes ──────────────────────────
//     useEffect(() => {
//         setSelectedCircles([]);
//     }, [fromMonth, toMonth, activeTech]);
...
//     // ✅ All available circles (union of both chart series names)
//     const allCircles = useMemo(() => {
//         const combined = new Set([...performanceSeries, ...offeredSeries]);
//         return Array.from(combined).sort();
//     }, [performanceSeries, offeredSeries]);
...
//     const isCircleAllSelected = selectedCircles.length === 0;
...
//                     {/* ✅ Circle filter + Category filter bar */}
//                     <Box sx={{
//                         display: "flex",
//                         alignItems: "center",
//                         gap: 1.5,
...
//                             themeColor={metricTheme.tabColor}
//                         />

//                         {/* Show selected circle chips */}
//                         {!isCircleAllSelected && (
//                             <Box display="flex" gap={0.6} flexWrap="wrap" alignItems="center">
...
//                                         size="small"
//                                         onDelete={() =>
//                                             setSelectedCircles((prev) => prev.filter((x) => x !== c))
//                                         }
//                                         sx={{
//                                             fontWeight: 700,
//                                             fontSize: 11,
//                                             bgcolor: alpha(metricTheme.tabColor, 0.1),
//                                             color: metricTheme.tabColor,
//                                             border: `1px solid ${alpha(metricTheme.tabColor, 0.3)}`,
//                                             "& .MuiChip-deleteIcon": { color: metricTheme.tabColor },
//                                         }}
//                                     />
//                                 ))}
//                                 <Chip
...
//                                 seriesInfo={performanceData?.graph_data?.[activeTech]?.series}
//                                 selectedCategories={selectedCategories}
//                                 selectedCircles={selectedCircles}   // ✅ passed down
//                             />
//                         )}
//                         {activeMetric === "offered" && (
//                             <AgingChart
...
//                         <Chip
//                             label={isCircleAllSelected ? "All" : `${selectedCircles.length} selected`}
//                             size="small"
//                             sx={{ background: metricTheme.tabColor, color: "#fff", fontWeight: 700, fontSize: 10.5 }}
//                         />
//                         <Typography variant="caption" color="text.secondary" fontWeight={700} ml={0.5}>Filter:</Typography>
