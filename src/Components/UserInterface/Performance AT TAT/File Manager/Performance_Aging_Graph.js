...
// ─────────────────────────────────────────────────────────────────────────────
// LABEL OFFSET COMPUTATION — safe for 0, 1, or N series
// ─────────────────────────────────────────────────────────────────────────────
/**
 * Given all non-null { seriesIdx, value } entries at one x-index,
 * returns { [seriesIdx]: yOffset } so labels don't overlap.
 * Fully guarded — never crashes on empty input.
 */
const computeLabelOffsets = (allValues) => {
    ...
    for (let i = 1; i < sorted.length; i++) {
        ...
        const curr = sorted[i];
        if (curr.value - prev.value < CLUSTER_THRESHOLD) {
            current.push(curr);
        } else {
    ...
    clusters.forEach((cluster) => {
        ...
        // Build alternating above/below slots
        const positions = [];
...
// ─────────────────────────────────────────────────────────────────────────────
// CUSTOM DOT — circle + pill label, collision-safe
// ─────────────────────────────────────────────────────────────────────────────
const CustomDot = (props) => {
    ...
    // Guard: skip rendering if coordinates or value are missing/invalid
        ...
        valuesAtIndex = Array.isArray(allSeriesData)
            ? allSeriesData
                .map((seriesValues, sIdx) => ({
                    seriesIdx: sIdx,
                    value: Array.isArray(seriesValues) ? (seriesValues[index] ?? null) : null,
                }))
    ...
    // Connector line: only draw when label is pushed far from dot
...
const AgingChart = ({
    ...
    // ── Filter — robust normalization so "<12%" matches "<12%" regardless of
    //    trailing spaces or case differences from the API
