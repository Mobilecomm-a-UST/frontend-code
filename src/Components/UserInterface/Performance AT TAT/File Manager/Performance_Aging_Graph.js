...
//     return (
//         <Paper elevation={4} sx={{ p: 1.5, minWidth: 180, borderRadius: 2, border: "1px solid #e0e0e0" }}>
...
// // ── Chart Panel ───────────────────────────────────────────────────────────────
// const AgingChart = ({ title, icon, gradientFrom, gradientTo, chartData, seriesNames, loading, seriesInfo }) => {
...
//                         <BarChart data={chartData} margin={{ top: 22, right: 16, left: 0, bottom: 10 }} barCategoryGap="25%" barGap={2}>
//                             <defs>
//                                 {seriesNames.map((name, i) => (
//                                     <linearGradient key={name} id={`grad_${chartId}_${i}`} x1="0" y1="0" x2="0" y2="1">
//                                         <stop offset="0%"   stopColor={SERIES_PALETTE[i % SERIES_PALETTE.length]} stopOpacity={0.95} />
//                                         <stop offset="100%" stopColor={SERIES_PALETTE[i % SERIES_PALETTE.length]} stopOpacity={0.65} />
//                                     </linearGradient>
//                                 ))}
//                             </defs>
...
//                             {seriesNames.map((name, i) => (
//                                 <Bar key={name} dataKey={name} fill={`url(#grad_${chartId}_${i})`} radius={[3, 3, 0, 0]} maxBarSize={28}>
//                                     <LabelList
//                                         dataKey={name}
//                                         position="top"
//                                         formatter={(v) => (v > 0 ? `${v}` : "")}
...
//     const [selectedMonth, setSelectedMonth] = useState(apiMonthToInput(getCurrentMonthStr()));
...
//         const monthParam = inputToApiMonth(selectedMonth);

//         setLoadingOffered(true);
//         const formOffered = new FormData();
//         formOffered.append("month", monthParam);
//         postData("performance_idploy/generate-offered-graph/", formOffered)
//             .then((res) => setOfferedData(res?.status ? res : null))
//             .catch(() => setOfferedData(null))
//             .finally(() => setLoadingOffered(false));

//         setLoadingPerformance(true);
//         const formPerf = new FormData();
//         formPerf.append("month", monthParam);
//         postData("performance_idploy/generate-performance-graph/", formPerf)
//             .then((res) => setPerformanceData(res?.status ? res : null))
...
//     // ── Derive chart data for active tech ─────────────────────────────────
...
//                             inputProps={{ max: apiMonthToInput(getCurrentMonthStr()) }}
//                             sx={{ minWidth: 160 }}
//                         />
...
//                         <Chip
//                             label={inputToApiMonth(selectedMonth)}
//                             size="small"
...
//                         <Chip
//                             label={`Offered: ${offeredSeries.length}  |  Performance: ${performanceSeries.length}`}
//                             size="small"
//                             variant="outlined"
            ...
            <Box sx={{ p: 2, background: "#fafafa", minHeight: 340 }}>
                {loading ? (
                    ...
                    <ResponsiveContainer width="100%" height={360}>
                        <BarChart data={chartData} margin={{ top: 22, right: 16, left: 0, bottom: 10 }} barCategoryGap="22%" barGap={3}>
                            <defs>
                                {seriesNames.map((name, i) => {
...
const Performance_Aging_Graph = () => {
    ...
    const fetchAll = useCallback(() => {
        ...
        const offeredPromises = selectedMonths.map((m) => {
            const form = new FormData();
            form.append("month", inputToApiMonth(m));
            return postData("performance_idploy/generate-offered-graph/", form)
        ...
        const perfPromises = selectedMonths.map((m) => {
            const form = new FormData();
            form.append("month", inputToApiMonth(m));
            return postData("performance_idploy/generate-performance-graph/", form)
                .then((res) => (res?.status ? res : null))
                .catch(() => null);
        });

        Promise.all(offeredPromises)
                ...
                <Box display="flex" justifyContent="space-between" alignItems="center" flexWrap="wrap" gap={1} mb={2}>
                    ...
                    <Box display="flex" gap={1} alignItems="center" flexWrap="wrap">
                        <MonthRangePicker
                            ...
                            onFromChange={(val) => {
                                setFromMonth(val);
                                // Auto-clamp toMonth if it goes below fromMonth
