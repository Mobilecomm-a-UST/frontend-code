...
//                     Performance SR Wise Aging Dashboard
//                 </Typography>
...
// ── Fallback "month" helper ────────────────────────────────────────────────
// Used only when no startDate/endDate props are supplied (e.g. this dashboard
// is opened before the Detailed Report tab has ever been visited/loaded).
// The backend accepts either {"month": "Dec 2025"} or {"start_date", "end_date"}.
const getCurrentMonthLabel = () => {
    const d = new Date();
    return d.toLocaleString("en-US", { month: "short", year: "numeric" }); // "Jul 2026"
};
...
//
// `startDate` / `endDate` (format "YYYY-MM-DD") are optional props — when the
// parent (e.g. Performance_SR_Wise_Main) supplies them, this dashboard sends
// the exact same range as the Detailed Report tab and re-fetches whenever they
// change, keeping both dashboards in sync. If they're not supplied, it falls
// back to the current calendar month so the component still works standalone.
const Performance_SR_Wise_summary3 = ({ startDate, endDate } = {}) => {
    const { loading, action } = useLoadingDialog();
    ...
    const fetchSummary = async () => {
        try {
            ...
            if (startDate && endDate) {
                // Mirror the exact range currently selected in the Detailed Report tab
                formData.append("start_date", startDate);
                formData.append("end_date", endDate);
            } else {
                // No range supplied yet (standalone use) — fall back to current month
                formData.append("month", getCurrentMonthLabel());
            }
    ...
    // ── Re-fetch whenever the shared date range changes (debounced, same as
    // the Detailed Report tab, so rapid date edits don't fire a call per keystroke) ──
    useEffect(() => {
    ...
    return (
        <Box p={1}>
            ...
            <Box display="flex" justifyContent="space-between" alignItems="center" flexWrap="wrap" gap={1}>
                ...
                {/* ── Only a download button, no date filters (range comes from the Detailed Report tab) ── */}
                <IconButton
                    onClick={handleDownload}
                    title="Download Excel"
                    disabled={!summaryResponse?.download_url}
