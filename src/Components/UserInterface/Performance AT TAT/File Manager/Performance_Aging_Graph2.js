...
// // MAIN COMPONENT
// // ─────────────────────────────────────────────────────────────────────────────
// const Performance_Aging_Graph2 = () => {
...
// export const MemoPerformance_Aging_Graph2 = React.memo(Performance_Aging_Graph2);
...
const Performance_Aging_Graph2 = () => {
    ...
    const fetchAll = useCallback(() => {
        ...
        // ── Performance ──
        Promise.all([
            Promise.all(selectedMonths.map((m) => fetchMonth("performance_idploy/generate-performance-graph/", m))),
            Promise.all(selectedMonths.map((m) => fetchMonth("performance_idploy/generate-performance-circle-graph/", m))),
        ...
        // ── Offered ──
        Promise.all([
            Promise.all(selectedMonths.map((m) => fetchMonth("performance_idploy/generate-offered-graph/", m))),
            Promise.all(selectedMonths.map((m) => fetchMonth("performance_idploy/generate-offered-circle-graph/", m))),
        ]).then(([mainResults, circleResults]) => {
            if (requestIdRef.current !== myId) return;
