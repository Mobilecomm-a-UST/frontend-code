...
import { postData, getData, deleteData } from "../../../services/FetchNodeServices";
import { useLoadingDialog } from "../../../Hooks/LoadingDialog";
// ✅ NEW: needed to read the logged-in user's role(s) so we can restrict
// the visible folder list for the PAT_5GKPI role to "KPI Trend" only.
...
const File_Manager = () => {
    const [open2, setOpen2] = useState(false);
        ...
        // ✅ NEW: PAT_5GKPI users only get access to the "KPI Trend" folder
        // (the isMultiVendor entry) — the other folders are hidden for them.
        const userTypes = (getDecreyptedData('user_type')?.split(","));
        const only5GKPI = userTypes?.includes('PAT_5GKPI');
        const visibleFolders = only5GKPI ? jsonData.filter((item) => item.isMultiVendor) : jsonData;
  ...
  return (
    <> 
            ...
            <Box sx={{ margin: '20px' }}>
                <Grid container rowSpacing={2} columnSpacing={3} direction={{ xs: "column", sm: "column", md: "row" }}>
                    {/* ✅ CHANGED: jsonData → visibleFolders (filtered to KPI Trend only for PAT_5GKPI) */}
