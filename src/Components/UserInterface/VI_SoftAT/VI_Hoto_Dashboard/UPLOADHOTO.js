...
const UPLOADHOTO = () => {
    ...
    const handleSubmit = async () => {
        ...
        if (rawKpiLength > 0) {
            ...
            try {
                const response = await postDataa(`ix_tracker_vi/upload_HOTO/`, formData);
    ...
    useEffect(() => {
        const fetchDownloadTemp = async () => {
            const res = await getData('ix_tracker_vi/HOTO_template/', { headers: { Authorization: `token ${getDecreyptedData("tokenKey")}` } })
    ...
    return (
        ...
        >
            <div>
                <div style={{ margin: 10, marginLeft: 10 }}>
                    <Breadcrumbs aria-label="breadcrumb" itemsBeforeCollapse={2} maxItems={3} separator={<KeyboardArrowRightIcon fontSize="small" />}>
                        ...
                        <Typography color='text.primary'>Upload VI File</Typography>
                    </Breadcrumbs>
                </div>
                ...
                <Box className={classes.main_Box}>
                    <Box className={classes.Back_Box}>
                        <Box className={classes.Box_Hading}>
                            Upload VI Integration HOTO Tracker File
                        </Box>
                        <Stack spacing={2} sx={{ marginTop: "-40px" }}>
 
                            <Box className={classes.Front_Box} id="step-1">
                                <div className={classes.Front_Box_Hading}>
                                    Select VI Integration HOTO Tracker {version.toUpperCase()} File:-<span style={{ fontFamily: 'Poppins', color: "gray", marginLeft: 20 }}>{rawKpiFile.filename}</span>
                                </div>
          ...
          <a download_link href={link}><Button variant="outlined" onClick='' startIcon={<FileDownloadIcon style={{ fontSize: 30, color: "green" }} />} sx={{ marginTop: "10px", width: "auto" }}><span style={{ fontFamily: "Poppins", fontSize: "22px", fontWeight: 800, textTransform: "none", textDecorationLine: "none" }}>Download KPI Trend</span></Button></a>
        </Box> */}
