...
const VI4G = () => {
    ...
    const handleSubmit = async () => {
        if (selectedFiles.length > 0) {
            ...
            const response = await postData('vi_gpl_4g/vi_slicing/', formData)
        ...
        <>
            <div style={{ margin: 5, marginLeft: 10 }}>
                <Breadcrumbs aria-label="breadcrumb" itemsBeforeCollapse={2} maxItems={3} separator={<KeyboardArrowRightIcon fontSize="small" />}>
                    ...
                    <Link underline="hover" onClick={() => { navigate('/tools/soft_at_tools') }}>VI GPL</Link>
                    <Typography color='text.primary'>VI 4G</Typography>
                </Breadcrumbs>
            </div>
            <Slide
                direction='left'
