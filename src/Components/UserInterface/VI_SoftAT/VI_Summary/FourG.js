...
const DOWNLOAD_LABELS = {
    download_url1: "Reference Data 4G",
    download_url2: "Inventory Data 4G",
    download_url3: "RAN UIM BTS Data 4G",
};
...
const FourG = () => {
    ...
    const handleSubmit = async () => {
        if (make4GFiles.length > 0 && selectCircle !== '') {
            ...
            const response = await postData('vi_summary/vi_summary_4g/', formData)

            if (response?.status === true) {
                action(false)
                setDownloadLinks({
                    download_url1: response.download_url1,
                    download_url2: response.download_url2,
                    download_url3: response.download_url3,
                })
                Swal.fire({
                    icon: "success",
                    title: "Done",
