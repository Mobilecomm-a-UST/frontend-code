...
const UploadFile = () => {
    ...
    const handleDownloadRelocation = async () => {
        try {
            ...
            const response = await postData('vi_ftr/download_viftr_file/', {
                userId: userID,
                year: year
            });
    ...
    const handleSubmit = async () => {
        if (make4GFiles) {
            ...
            formData.append(`ftr_file`, make4GFiles.bytes);
            formData.append(`userId`, userID);
            const response = await postData('vi_ftr/upload_viftr/', formData)
                        ...
                        <Stack spacing={2} sx={{ marginTop: "-40px" }} direction={'column'}>
                            ...
                            <Box className={classes.Front_Box} >
                                <div className={classes.Front_Box_Hading}>
                                    Select FTR Files:-<span style={{ fontFamily: 'Poppins', color: "gray", marginLeft: 20 }}>{make4GFiles.filename}</span>
                                </div>
