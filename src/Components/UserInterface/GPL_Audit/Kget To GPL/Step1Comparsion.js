...
const Step1Comparsion = () => {
    ...
    const handleSubmit = async () => {
        if (make4GFiles.length >  0 && postFiles.length > 0) {
            action(true)
            var formData = new FormData();
             for (let i = 0; i < make4GFiles.length; i++) {
                // console.log('pre files' , preFiles[i])
                formData.append(`GPL_FILE`, make4GFiles[i]);
            }
            for (let i = 0; i < postFiles.length; i++) {
                // console.log('post files' , postFiles[i])
                formData.append(`LOG_FILE`, postFiles[i]);
            }


            const response = await postDataa('gpl_audit_tool_V2/comperison_kget_to_gpl/', formData)
        ...
        <>
            <div style={{ margin: 5, marginLeft: 10 }}>
                <Breadcrumbs aria-label="breadcrumb" itemsBeforeCollapse={2} maxItems={3} separator={<KeyboardArrowRightIcon fontSize="small" />}>
                    ...
                    <Typography color='text.primary'>Step-1-Comparison</Typography>
                </Breadcrumbs>
            </div>
            <Slide
                direction='left'
                    ...
                    <Box className={classes.main_Box}>
                        <Box className={classes.Back_Box} sx={{ width: { md: '75%', xs: '100%' } }}>
                            <Box className={classes.Box_Hading} >
                               KGET to GPL Comparison
                            </Box>
                            <Stack spacing={2} sx={{ marginTop: "-40px" }} direction={'column'}>

                                <Box className={classes.Front_Box} >
                                    <div className={classes.Front_Box_Hading}>
                                        Select Excel File:-<span style={{ fontFamily: 'Poppins', color: "gray", marginLeft: 20 }}>{ }</span>
                                    </div>
                                    <div className={classes.Front_Box_Select_Button} >
                                        <div style={{ float: "left" }}>
                                            <Button variant="contained" component="label" color={make4GFiles.length > 0 ? "warning" : "primary"}>
                                                select file
                                                <input required hidden accept=".xlsx,.xls"  type="file"
                                ...
                                <Box className={classes.Front_Box} >
                                    <div className={classes.Front_Box_Hading}>
                                        Select Log/Text File:-<span style={{ fontFamily: 'Poppins', color: "gray", marginLeft: 20 }}>{ }</span>
                                    </div>
                                    <div className={classes.Front_Box_Select_Button} >
                                        <div style={{ float: "left" }}>
                                            <Button variant="contained" component="label" color={make4GFiles.length > 0 ? "warning" : "primary"}>
                                                ...
                                                <input required hidden accept=".log,.txt"  type="file"
                                                    onChange={(e) => { setPostFiles(e.target.files); setShowPost(false); }} />
                                            </Button>
                                        </div>

                                        {postFiles.length > 0 && <span style={{ color: 'green', fontSize: '18px', fontWeight: 600 }}>Selected File(s) : {make4GFiles.length}</span>}
...
export default Step1Comparsion