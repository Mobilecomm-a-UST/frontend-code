...
const Step2Scripting = () => {
    ...
    const handleSubmit = async () => {
        if (make4GFiles.length) {
            action(true)
            var formData = new FormData();
             for (let i = 0; i < make4GFiles.length; i++) {
                // console.log('pre files' , preFiles[i])
                formData.append(`REPORT_FILE`, make4GFiles[i]);
            }
           


            const response = await postDataa('gpl_audit_tool_V2/scripter_kget_to_gpl/', formData)
        ...
        <>
            <div style={{ margin: 5, marginLeft: 10 }}>
                <Breadcrumbs aria-label="breadcrumb" itemsBeforeCollapse={2} maxItems={3} separator={<KeyboardArrowRightIcon fontSize="small" />}>
                    ...
                    <Typography color='text.primary'>Step-1-Scripting</Typography>
                </Breadcrumbs>
            </div>
            <Slide
                direction='left'
