...
const Kpi4G = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (rawKpiLength > 0 && siteListLength > 0) {
      ...
      formData.append("4G_raw_kpi", rawKpiFile.bytes);
      formData.append("site_list_4G", siteListFile.bytes);
      formData.append("offered date", offerDate);
      // const response = await postData('trend/jk/makeKpiTrend/old/',formData, {headers: {Authorization: `token ${JSON.parse(localStorage.getItem("tokenKey"))}` },signal:abortSignal})
      const response = await makePostRequest('trend/jk/makeKpiTrend/old/', formData)
      ...
      <div>
        <div style={{ margin: 10, marginLeft: 10 }}>
          <Breadcrumbs aria-label="breadcrumb" itemsBeforeCollapse={2} maxItems={3} separator={<KeyboardArrowRightIcon fontSize="small" />}>
            ...
            <Link underline="hover" href='/tools/quality_team/trends/jk'>JK</Link>
