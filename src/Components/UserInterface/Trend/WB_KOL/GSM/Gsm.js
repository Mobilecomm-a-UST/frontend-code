...
const startTour = () => {
    ...
    const handleSubmit = async () => {
        ...
        if (rawKpiLength > 0 && siteListLength > 0) {
            ...
            // const response = await postData('trend/kol/makeKpiTrend/old/gsm', formData, { headers: { Authorization: `token ${JSON.parse(localStorage.getItem("tokenKey"))}` }
            // ,signal:abortSignal})
            const response = await makePostRequest('trend/kol/makeKpiTrend/old/gsm', formData)
            ...
            <div>
                <div style={{ margin: 10, marginLeft: 10 }}>
                    <Breadcrumbs aria-label="breadcrumb" itemsBeforeCollapse={2} maxItems={3} separator={<KeyboardArrowRightIcon fontSize="small" />}>
                        ...
                        <Typography color='text.primary'>GSM</Typography>
                    </Breadcrumbs>
                </div>
