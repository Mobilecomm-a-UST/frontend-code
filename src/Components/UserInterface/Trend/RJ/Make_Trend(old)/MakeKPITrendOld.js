...
const startTour = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (rawKpiLength > 0 && siteListLength > 0 || siteData.length > 0) {
      ...
      // const response = await postData('trend/raj/makeKpiTrend/old', formData, { headers: { Authorization: `token ${JSON.parse(localStorage.getItem("tokenKey"))}` }
      const response = await makePostRequest('trend/raj/makeKpiTrend/old', formData)
      ...
      <div>
        <div style={{ margin: 10, marginLeft: 10 }}>
          <Breadcrumbs aria-label="breadcrumb" itemsBeforeCollapse={2} maxItems={3} separator={<KeyboardArrowRightIcon fontSize="small" />}>
            ...
            <Link underline="hover" href='/trends/rj'>RJ</Link>
