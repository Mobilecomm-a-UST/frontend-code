...
const FourG = () => {
  ...
  console.log('new files' , rawKpiFile)
  ...
  const handleSubmit = async () => {
    ...
    if (rawKpiLength > 0 && siteListLength > 0 || siteData.length > 0) {
      ...
      // const response = await postData('trend/ap/makeKpiTrend/old/4G', formData, { headers: { Authorization: `token ${JSON.parse(localStorage.getItem("tokenKey"))}` },signal : abortSignal})
      const response = await makePostRequest('trend/ap/makeKpiTrend/old/4G', formData )
      ...
      <div>
        <div style={{ margin: 10, marginLeft: 10 }}>
          <Breadcrumbs aria-label="breadcrumb" itemsBeforeCollapse={2} maxItems={3} separator={<KeyboardArrowRightIcon fontSize="small" />}>
            ...
            <Typography color='text.primary'>4G</Typography>
          </Breadcrumbs>
        </div>
