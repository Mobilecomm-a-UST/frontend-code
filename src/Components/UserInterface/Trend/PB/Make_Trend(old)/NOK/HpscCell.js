...
const startTour = () => {
    const driverObj = driver({
      ...
      showProgress: true,
      steps: [
        { element: '#step-1', popover: { title: 'Select 4G RAW KPI File', description: 'Select 4G RAW KPI File generated from MyCom Tool', side: "bottom", align: 'start' } },
        { element: '#step-2', popover: { title: 'Select 4G Site List', description: 'Select 4G Site List in excel formate having column name 2G ID', side: "left", align: 'start' } },
    ...
    const handleSubmit = async () => {
        ...
        if (rawKpiLength > 0 && siteListLength > 0) {
            ...
            formData.append("raw_kpi_4G", rawKpiFile.bytes);
            formData.append("site_list_4G", siteListFile.bytes);
            formData.append("offered_date", offerDate);
            // const response = await postData('trend/pb/makeKpiTrend/nok/hpsccell/old', formData, { headers: { Authorization: `token ${JSON.parse(localStorage.getItem("tokenKey"))}` } ,signal:abortSignal})
            const response = await makePostRequest('trend/pb/makeKpiTrend/nok/hpsccell/old', formData)
                    ...
                    <Breadcrumbs aria-label="breadcrumb" itemsBeforeCollapse={2} maxItems={3} separator={<KeyboardArrowRightIcon fontSize="small" />}>
                        <Link underline="hover" href='/tools'>Tools</Link>
                                          ...
                                          <Link underline="hover" href='/tools/quality_team/trends'>Trends</Link>
                                           <Link underline="hover" href='/tools/quality_team/trends/pb'>PB</Link>
                        <Typography color='text.primary'>HPSC Cell</Typography>
                    </Breadcrumbs>
                </div>
                ...
                <Box className={classes.main_Box}>
                    <Box className={classes.Back_Box}>
                        <Box className={classes.Box_Hading}>
                            Make KPI TREND NOC-HPSC Cell(OLD)
                        </Box>
                        <Stack spacing={2} sx={{ marginTop: "-40px" }}>
                            ...
                            <Box className={classes.Front_Box}id='step-2'>
                                <div className={classes.Front_Box_Hading}>
                                    Select 4G Site List:-<span style={{ fontFamily: 'Poppins', color: "gray", marginLeft: 20 }}>{siteListFile.filename}</span>
                                </div>
                                <div className={classes.Front_Box_Select_Button}>
                                    <div style={{ float: "left" }}>
