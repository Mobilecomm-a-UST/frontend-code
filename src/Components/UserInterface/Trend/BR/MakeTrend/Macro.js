...
const Macro = () => {
    ...
    const handleSubmit = async () => {
      ...
      if (rawKpiLength > 0 && siteListLength > 0 || siteData.length > 0) {
        ...
        const response = await makePostRequest('trend/bih/makeKpiTrend/old/macro', formData )

        // const response = await postData('trend/bih/makeKpiTrend/old/macro', formData, { headers: { Authorization: `token ${JSON.parse(localStorage.getItem("tokenKey"))}` },signal:abortSignal })
