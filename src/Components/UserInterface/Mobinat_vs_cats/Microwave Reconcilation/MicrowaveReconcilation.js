...
const MicrowaveReconcilation = () => {
  // Only two user-selected uploads on this screen: Microwave Report File and
  // Hardware File. Manual File has been removed entirely (state, validation,
  // formData, and cancel-reset all dropped — not just hidden from the UI).
  ...
  const fetchMobinetFileData = async () => {
    ...
    // Mobinet Dump files (not shown on this screen) and MO-VS-CAP files
    // (for the last card).
  ...
  const handleSubmit = async () => {
    ...
    formData.append("mw_file", siteList.bytes);
    formData.append("hw_file", hardWareFile.bytes);

    const response = await postData("mobinate_vs_cats/mw_dismental/", formData);
    action(false);
        ...
        <Box>
          <Box className={classes.main_Box}>
            <Box className={classes.Back_Box} sx={{ width: { md: "75%", xs: "100%" } }}>
              <Box className={classes.Box_Hading}>Microwave Reconcilation</Box>

              <Stack spacing={2} sx={{ mt: "-40px" }}>
                {/* Site List */}
                <UploadSection
                  label="Select Microwave Report File"
      ...
      <div className={OverAllCss().Front_Box_Select_Button}>
        <Button variant="contained" component="label" color={color}>
          ...
          <input
            ...
            required
            type="file"
            accept=".xlsx,.xls,.csv,.xlsb, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.ms-excel"
            multiple={multiple}
            onChange={onChange}
          />
        </Button>
...
export default MicrowaveReconcilation