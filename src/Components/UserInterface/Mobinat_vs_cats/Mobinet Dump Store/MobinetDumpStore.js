...
const MobinetDB = () => {
    ...
    // ✅ NEW: Reference download loading state
    const [referenceDownloadLoading, setReferenceDownloadLoading] = useState(false);
    ...
    // ✅ NEW: Download Mobinate Reference Function
    const handleDownloadMobinateReference = async () => {
        setReferenceDownloadLoading(true);

        try {
            const response = await postData("mobinate_vs_cats/mobinet_db_summary_export/", {});

            if (response && response.download_url) {
                const link = document.createElement("a");
                link.href = response.download_url;
                link.download = response.filename || "Mobinate_Reference.xlsx";
                link.target = "_blank";
                ...
                Swal.fire({
                    icon: "success",
                    title: "Downloaded",
                    text: "Mobinate reference downloaded successfully",
                });
            } else {
                Swal.fire({
                    icon: "error",
                    title: "Download Failed",
                    text: response?.message || "Failed to download Mobinate reference",
                });
            }
        } catch (error) {
            ...
            Swal.fire({
                icon: "error",
                title: "Error",
                text: error?.message || "Error downloading Mobinate reference",
            });
        } finally {
            setReferenceDownloadLoading(false);
        }
    };
        ...
        <>
            ...
            <Box sx={{ position: "fixed", top: 70, right: 30, zIndex: 1200 }}>
                <Tooltip title="Download Mobinate Reference">
                    <Button
                        variant="contained"
                        startIcon={referenceDownloadLoading ? undefined : <DownloadForOfflineIcon />}
                        onClick={handleDownloadMobinateReference}
                        disabled={referenceDownloadLoading}
                        sx={{
                            background: "#1976D2",
                            color: "#fff",
                            fontWeight: 700,
