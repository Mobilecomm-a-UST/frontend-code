...
const UploadFile = () => {
    ...
    const handleSubmit = async () => {
        ...
        if (rawKpiLength > 0) {
            ...
            try {
                const response = await postDataa(`ix_tracker_vi/upload/`, formData);
    ...
    useEffect(() => {
        const fetchDownloadTemp = async () => {
            const res = await getData('ix_tracker_vi/template/', { headers: { Authorization: `token ${getDecreyptedData("tokenKey")}` } })
                ...
                <Box className={classes.main_Box}>
                    <Box className={classes.Back_Box}>
                        <Box className={classes.Box_Hading}>
                            Upload VI Integration Tracker File
                        </Box>
                        <Stack spacing={2} sx={{ marginTop: "-40px" }}>

                            <Box className={classes.Front_Box} id="step-1">
                                <div className={classes.Front_Box_Hading}>
                                    Select VI Integration Tracker {version.toUpperCase()} File:-<span style={{ fontFamily: 'Poppins', color: "gray", marginLeft: 20 }}>{rawKpiFile.filename}</span>
                                </div>
