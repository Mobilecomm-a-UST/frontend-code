...
const MonthWiseIntegration = ({ onData }) => {
    ...
    // ✅ DE-GROW re-enabled + existing activities
    ...
    const ShortDate = (months, years) => {
        const result = [];
        for (let i = 0; i < Math.min(months?.length, years?.length); i++) {
            result.push({ month: months[i], year: years[i] });
        }
        setMonthArray(result)
    }
    ...
    const handleCellClick = (value, clickParams) => {
        ...
        if (numVal === 0) return;   // disabled — no action
        ClickDataGet(clickParams);
    }

    // ✅ Helper: cell style — dim + no-pointer when value is 0
            ...
            <Slide direction="left" in='true' timeout={700} style={{ transformOrigin: '1 1 1' }}>
                <div style={{ margin: 20 }}>
                    ...
                    <Box style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', alignContent: 'center' }}>
                        ...
                        <Box>
                            {/* ✅ Future dates disabled via maxDate */}
                            <LocalizationProvider dateAdapter={AdapterDayjs}>
                                <DatePicker
                                    views={['month', 'year']}
                            ...
                            <table style={{ width: "100%", border: "1px solid black", borderCollapse: 'collapse', overflow: 'auto' }} >
                                <thead style={{ position: 'sticky', top: 0, zIndex: 1 }}>
                                    <tr style={{ fontSize: 15, backgroundColor: "#223354", color: "white", border: '1px solid white' }}>
                                        <th rowSpan='2' style={{ padding: '5px 20px', whiteSpace: 'nowrap', position: 'sticky', left: 0, top: 0, backgroundColor: '#223354' }}>CIRCLE</th>
                                        {monthArray?.map((item, index) => index < 3 && (
                                            <th colSpan='14' key={index} style={{ padding: '5px 20px', whiteSpace: 'nowrap', backgroundColor: headerColor[index] }}>
                                    ...
                                    {tableData?.map((it, index) => {
                                        return (
                                            <tr className={classes.hover} key={index} style={{ textAlign: "center", fontWeigth: 700 }}>
                                                ...
                                                {/* ── M1 ── */}
                                                <th className={classes.hover} style={cellStyle(it?.M1_DE_GROW)} onClick={() => handleCellClick(it?.M1_DE_GROW, { mont: months[0], yea: years[0], circle: it?.cir, activity: 'DE-GROW' })}>{it?.M1_DE_GROW}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M1_MACRO)} onClick={() => handleCellClick(it?.M1_MACRO, { mont: months[0], yea: years[0], circle: it?.cir, activity: 'MACRO' })}>{it?.M1_MACRO}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M1_RELOCATION)} onClick={() => handleCellClick(it?.M1_RELOCATION, { mont: months[0], yea: years[0], circle: it?.cir, activity: 'RELOCATION' })}>{it?.M1_RELOCATION}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M1_ULS_HPSC)} onClick={() => handleCellClick(it?.M1_ULS_HPSC, { mont: months[0], yea: years[0], circle: it?.cir, activity: 'ULS_HPSC' })}>{it?.M1_ULS_HPSC}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M1_UPGRADE)} onClick={() => handleCellClick(it?.M1_UPGRADE, { mont: months[0], yea: years[0], circle: it?.cir, activity: 'UPGRADE' })}>{it?.M1_UPGRADE}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M1_RRU_UPGRADE)} onClick={() => handleCellClick(it?.M1_RRU_UPGRADE, { mont: months[0], yea: years[0], circle: it?.cir, activity: 'RRU UPGRADE' })}>{it?.M1_RRU_UPGRADE || 0}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M1_5G_SECTOR_ADDITION)} onClick={() => handleCellClick(it?.M1_5G_SECTOR_ADDITION, { mont: months[0], yea: years[0], circle: it?.cir, activity: '5G SECTOR ADDITION' })}>{it?.M1_5G_SECTOR_ADDITION}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M1_5G_RELOCATION)} onClick={() => handleCellClick(it?.M1_5G_RELOCATION, { mont: months[0], yea: years[0], circle: it?.cir, activity: '5G RELOCATION' })}>{it?.M1_5G_RELOCATION}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M1_RRU_SWAP)} onClick={() => handleCellClick(it?.M1_RRU_SWAP, { mont: months[0], yea: years[0], circle: it?.cir, activity: 'RRU SWAP' })}>{it?.M1_RRU_SWAP}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M1_FR_Date)} onClick={() => handleCellClick(it?.M1_FR_Date, { mont: months[0], yea: years[0], circle: it?.cir, activity: 'FR_Date' })}>{it?.M1_FR_Date}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M1_HOTO_Offered_2g)} onClick={() => handleCellClick(it?.M1_HOTO_Offered_2g, { mont: months[0], yea: years[0], circle: it?.cir, activity: 'HOTO_Offered_2g' })}>{it?.M1_HOTO_Offered_2g}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M1_HOTO_Accepted_2g)} onClick={() => handleCellClick(it?.M1_HOTO_Accepted_2g, { mont: months[0], yea: years[0], circle: it?.cir, activity: 'HOTO_Accepted_2g' })}>{it?.M1_HOTO_Accepted_2g}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M1_HOTO_Offered_4g)} onClick={() => handleCellClick(it?.M1_HOTO_Offered_4g, { mont: months[0], yea: years[0], circle: it?.cir, activity: 'HOTO_Offered_4g' })}>{it?.M1_HOTO_Offered_4g}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M1_HOTO_Accepted_4g)} onClick={() => handleCellClick(it?.M1_HOTO_Accepted_4g, { mont: months[0], yea: years[0], circle: it?.cir, activity: 'HOTO_Accepted_4g' })}>{it?.M1_HOTO_Accepted_4g}</th>

                                                {/* ── M2 ── */}
                                                <th className={classes.hover} style={cellStyle(it?.M2_DE_GROW)} onClick={() => handleCellClick(it?.M2_DE_GROW, { mont: months[1], yea: years[1], circle: it?.cir, activity: 'DE-GROW' })}>{it?.M2_DE_GROW}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M2_MACRO)} onClick={() => handleCellClick(it?.M2_MACRO, { mont: months[1], yea: years[1], circle: it?.cir, activity: 'MACRO' })}>{it?.M2_MACRO}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M2_RELOCATION)} onClick={() => handleCellClick(it?.M2_RELOCATION, { mont: months[1], yea: years[1], circle: it?.cir, activity: 'RELOCATION' })}>{it?.M2_RELOCATION}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M2_ULS_HPSC)} onClick={() => handleCellClick(it?.M2_ULS_HPSC, { mont: months[1], yea: years[1], circle: it?.cir, activity: 'ULS_HPSC' })}>{it?.M2_ULS_HPSC}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M2_UPGRADE)} onClick={() => handleCellClick(it?.M2_UPGRADE, { mont: months[1], yea: years[1], circle: it?.cir, activity: 'UPGRADE' })}>{it?.M2_UPGRADE}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M2_RRU_UPGRADE)} onClick={() => handleCellClick(it?.M2_RRU_UPGRADE, { mont: months[1], yea: years[1], circle: it?.cir, activity: 'RRU UPGRADE' })}>{it?.M2_RRU_UPGRADE || 0}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M2_5G_SECTOR_ADDITION)} onClick={() => handleCellClick(it?.M2_5G_SECTOR_ADDITION, { mont: months[1], yea: years[1], circle: it?.cir, activity: '5G SECTOR ADDITION' })}>{it?.M2_5G_SECTOR_ADDITION}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M2_5G_RELOCATION)} onClick={() => handleCellClick(it?.M2_5G_RELOCATION, { mont: months[1], yea: years[1], circle: it?.cir, activity: '5G RELOCATION' })}>{it?.M2_5G_RELOCATION}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M2_RRU_SWAP)} onClick={() => handleCellClick(it?.M2_RRU_SWAP, { mont: months[1], yea: years[1], circle: it?.cir, activity: 'RRU SWAP' })}>{it?.M2_RRU_SWAP}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M2_FR_Date)} onClick={() => handleCellClick(it?.M2_FR_Date, { mont: months[1], yea: years[1], circle: it?.cir, activity: 'FR_Date' })}>{it?.M2_FR_Date}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M2_HOTO_Offered_2g)} onClick={() => handleCellClick(it?.M2_HOTO_Offered_2g, { mont: months[1], yea: years[1], circle: it?.cir, activity: 'HOTO_Offered_2g' })}>{it?.M2_HOTO_Offered_2g}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M2_HOTO_Accepted_2g)} onClick={() => handleCellClick(it?.M2_HOTO_Accepted_2g, { mont: months[1], yea: years[1], circle: it?.cir, activity: 'HOTO_Accepted_2g' })}>{it?.M2_HOTO_Accepted_2g}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M2_HOTO_Offered_4g)} onClick={() => handleCellClick(it?.M2_HOTO_Offered_4g, { mont: months[1], yea: years[1], circle: it?.cir, activity: 'HOTO_Offered_4g' })}>{it?.M2_HOTO_Offered_4g}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M2_HOTO_Accepted_4g)} onClick={() => handleCellClick(it?.M2_HOTO_Accepted_4g, { mont: months[1], yea: years[1], circle: it?.cir, activity: 'HOTO_Accepted_4g' })}>{it?.M2_HOTO_Accepted_4g}</th>

                                                {/* ── M3 ── */}
                                                <th className={classes.hover} style={cellStyle(it?.M3_DE_GROW)} onClick={() => handleCellClick(it?.M3_DE_GROW, { mont: months[2], yea: years[2], circle: it?.cir, activity: 'DE-GROW' })}>{it?.M3_DE_GROW}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M3_MACRO)} onClick={() => handleCellClick(it?.M3_MACRO, { mont: months[2], yea: years[2], circle: it?.cir, activity: 'MACRO' })}>{it?.M3_MACRO}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M3_RELOCATION)} onClick={() => handleCellClick(it?.M3_RELOCATION, { mont: months[2], yea: years[2], circle: it?.cir, activity: 'RELOCATION' })}>{it?.M3_RELOCATION}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M3_ULS_HPSC)} onClick={() => handleCellClick(it?.M3_ULS_HPSC, { mont: months[2], yea: years[2], circle: it?.cir, activity: 'ULS_HPSC' })}>{it?.M3_ULS_HPSC}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M3_UPGRADE)} onClick={() => handleCellClick(it?.M3_UPGRADE, { mont: months[2], yea: years[2], circle: it?.cir, activity: 'UPGRADE' })}>{it?.M3_UPGRADE}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M3_RRU_UPGRADE)} onClick={() => handleCellClick(it?.M3_RRU_UPGRADE, { mont: months[2], yea: years[2], circle: it?.cir, activity: 'RRU UPGRADE' })}>{it?.M3_RRU_UPGRADE || 0}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M3_5G_SECTOR_ADDITION)} onClick={() => handleCellClick(it?.M3_5G_SECTOR_ADDITION, { mont: months[2], yea: years[2], circle: it?.cir, activity: '5G SECTOR ADDITION' })}>{it?.M3_5G_SECTOR_ADDITION}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M3_5G_RELOCATION)} onClick={() => handleCellClick(it?.M3_5G_RELOCATION, { mont: months[2], yea: years[2], circle: it?.cir, activity: '5G RELOCATION' })}>{it?.M3_5G_RELOCATION}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M3_RRU_SWAP)} onClick={() => handleCellClick(it?.M3_RRU_SWAP, { mont: months[2], yea: years[2], circle: it?.cir, activity: 'RRU SWAP' })}>{it?.M3_RRU_SWAP}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M3_FR_Date)} onClick={() => handleCellClick(it?.M3_FR_Date, { mont: months[2], yea: years[2], circle: it?.cir, activity: 'FR_Date' })}>{it?.M3_FR_Date}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M3_HOTO_Offered_2g)} onClick={() => handleCellClick(it?.M3_HOTO_Offered_2g, { mont: months[2], yea: years[2], circle: it?.cir, activity: 'HOTO_Offered_2g' })}>{it?.M3_HOTO_Offered_2g}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M3_HOTO_Accepted_2g)} onClick={() => handleCellClick(it?.M3_HOTO_Accepted_2g, { mont: months[2], yea: years[2], circle: it?.cir, activity: 'HOTO_Accepted_2g' })}>{it?.M3_HOTO_Accepted_2g}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M3_HOTO_Offered_4g)} onClick={() => handleCellClick(it?.M3_HOTO_Offered_4g, { mont: months[2], yea: years[2], circle: it?.cir, activity: 'HOTO_Offered_4g' })}>{it?.M3_HOTO_Offered_4g}</th>
                                                <th className={classes.hover} style={cellStyle(it?.M3_HOTO_Accepted_4g)} onClick={() => handleCellClick(it?.M3_HOTO_Accepted_4g, { mont: months[2], yea: years[2], circle: it?.cir, activity: 'HOTO_Accepted_4g' })}>{it?.M3_HOTO_Accepted_4g}</th>
                                            </tr>
                                        )
                                    })}
                                    <tr style={{ textAlign: "center", fontWeigth: 700, backgroundColor: '#B0EBB4', color: '#000000', fontSize: 17 }}>
