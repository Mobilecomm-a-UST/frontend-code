...
//         const responce = await makePostRequest('ix_tracker_vi/hyperlink-datewise-integration-data/', formData)
//         if (responce) {
                        ...
                        <TableContainer sx={{ maxHeight: 550, boxShadow: 'rgba(0, 0, 0, 0.24) 0px 3px 8px' }} component={Paper}>
                            <table style={{ width: "100%", border: "1px solid black", borderCollapse: 'collapse', overflow: 'auto' }}>
                                <thead style={{ position: 'sticky', top: 0, zIndex: 1 }}>
                                    ...
                                    <tr style={{ fontSize: 15, backgroundColor: "#223354", color: "white", border: '1px solid white' }}>
                                        {activityArray.map((item, index) => (
                                            <th key={`d1-${index}`} style={{ padding: '5px 20px', whiteSpace: 'nowrap', backgroundColor: '#5AB2FF' }}>{item}</th>
                                        ))}
                                        {activityArray.map((item, index) => (
                                            <th key={`d2-${index}`} style={{ padding: '5px 20px', whiteSpace: 'nowrap', backgroundColor: '#FF9933' }}>{item}</th>
                                        ))}
                                        {activityArray.map((item, index) => (
                                            <th key={`d3-${index}`} style={{ padding: '5px 20px', whiteSpace: 'nowrap', backgroundColor: '#68D2E8' }}>{item}</th>
                                        ))}
                                    </tr>
                                </thead>
                                <tbody>
