...
const TicketCounter = () => {
    ...
    const handleSubmit = async () => {
        ...
        if (rawKpiLength > 0) {
            ...
            formData.append("ticket_counter_file", rawKpiFile.bytes);
            const response = await makePostRequest('Zero_Count_Rna_Payload_Tool/ticket_counter_upload_report/', formData)
            // console.log('response ticket status', response)
            if (response) {
                setOpen(false);
                Swal.fire({
                    icon: "success",
                ...
                <Box className={classes.main_Box}>
                    <Box className={classes.Back_Box}>
                        <Box className={classes.Box_Hading}>
                            Upload Ticket Status Counter
                        </Box>
                        <Stack spacing={2} sx={{ marginTop: "-40px" }}>

                            <Box className={classes.Front_Box} id="step-1">
                                <div className={classes.Front_Box_Hading}>
                                    Select Ticket Status File:-<span style={{ fontFamily: 'Poppins', color: "gray", marginLeft: 20 }}>{rawKpiFile.filename}</span>
                                </div>
