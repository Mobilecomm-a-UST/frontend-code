...
const ProjectWise = () => {
    ...
    const getProjectFilterData = async () => {
        ...
        month?.map((items) => {
            ...
            if (lenghtArr === 0) {
                setNEW_TOWER_ULS((prev) => [...prev, 0]);
                setNEW_TOWER((prev) => [...prev, 0]);
                setNEW_SECTOR_ADDITION((prev) => [...prev, 0]);
                setM_MIMO((prev) => [...prev, 0]);
                setUPGRADE_SW_ONLY((prev) => [...prev, 0]);
                setFDD_UPGRADE((prev) => [...prev, 0]);
                setUPGRADE_ULS((prev) => [...prev, 0]);
                setFDD_TWIN_BEAM((prev) => [...prev, 0]);
                setL900_UPGRADE((prev) => [...prev, 0]);
                setOEM_SWAP((prev) => [...prev, 0]);
                setTDD_UPGRADE((prev) => [...prev, 0]);
                setG2G_UPGRADE((prev) => [...prev, 0]);
                setTDD_TWIN_BEAM((prev) => [...prev, 0]);
                setG2G_SEC_ADDITION((prev) => [...prev, 0]);
                setL2100_UPGRADE((prev) => [...prev, 0]);
                setFDD_SEC_ADDITION((prev) => [...prev, 0]);
                setTDD_SEC_ADDITION((prev) => [...prev, 0]);
            }
            else {
                ...
                filterCircle.map((it) => {
                    ...
                    if (it.project === 'NEW_TOWER_ULS') {
                        // setOverAll((prev) => [...prev, [NEW_TOWER_ULS]:it.Counting])
                        setNEW_TOWER_ULS((prev) => [...prev, it.Counting])
                        tempArr.push(it.project)
                        // console.warn('Mobilecomm for data')
                    }

                    else if (it.project === 'NEW_TOWER') {
                        setNEW_TOWER((prev) => [...prev, it.Counting])
                        tempArr.push(it.project)
                        // console.warn('Mobilecomm for data')
                    }
                    else if (it.project === 'NEW_SECTOR_ADDITION') {
                        setNEW_SECTOR_ADDITION((prev) => [...prev, it.Counting])
                        tempArr.push(it.project)
                        // console.warn('Mobilecomm for data')
                    }
                    else if (it.project === 'M_MIMO') {
                        setM_MIMO((prev) => [...prev, it.Counting])
                        tempArr.push(it.project)

                    }
                    else if (it.project === 'UPGRADE_SW_ONLY') {
                        setUPGRADE_SW_ONLY((prev) => [...prev, it.Counting])
                        tempArr.push(it.project)

                    }
                    else if (it.project === 'FDD_UPGRADE') {
                        setFDD_UPGRADE((prev) => [...prev, it.Counting])
                        tempArr.push(it.project)

                    }
                    else if (it.project === 'UPGRADE_ULS') {
                        setUPGRADE_ULS((prev) => [...prev, it.Counting])
                        tempArr.push(it.project)

                    }
                    else if (it.project === 'FDD_TWIN_BEAM') {
                        setFDD_TWIN_BEAM((prev) => [...prev, it.Counting])
                        tempArr.push(it.project)

                    }
                    else if (it.project === 'L900_UPGRADE') {
                        setL900_UPGRADE((prev) => [...prev, it.Counting])
                        tempArr.push(it.project)

                    }
                    else if (it.project === 'OEM_SWAP') {
                        setOEM_SWAP((prev) => [...prev, it.Counting])
                        tempArr.push(it.project)

                    }
                    else if (it.project === 'TDD_UPGRADE') {
                        setTDD_UPGRADE((prev) => [...prev, it.Counting])
                        tempArr.push(it.project)

                    }
                    else if (it.project === 'G2G_UPGRADE') {
                        setG2G_UPGRADE((prev) => [...prev, it.Counting])
                        tempArr.push(it.project)

                    }
                    else if (it.project === 'TDD_TWIN_BEAM') {
                        setTDD_TWIN_BEAM((prev) => [...prev, it.Counting])
                        tempArr.push(it.project)

                    }
                    else if (it.project === 'G2G_SEC_ADDITION') {
                        setG2G_SEC_ADDITION((prev) => [...prev, it.Counting])
                        tempArr.push(it.project)

                    }
                    else if (it.project === 'L2100_UPGRADE') {
                        setL2100_UPGRADE((prev) => [...prev, it.Counting])
                        tempArr.push(it.project)

                    }
                    else if (it.project === 'FDD_SEC_ADDITION') {
                        setFDD_SEC_ADDITION((prev) => [...prev, it.Counting])
                        tempArr.push(it.project)

                    }
                    else if (it.project === 'TDD_SEC_ADDITION') {
                        setTDD_SEC_ADDITION((prev) => [...prev, it.Counting])
                        tempArr.push(it.project)
                ...
                for (var i = 0; i < ProjectArrs.length; i++) {
                    if (!tempArr.includes(ProjectArrs[i])) {
                        switch (ProjectArrs[i]) {
                            case "NEW_TOWER_ULS":
                                setNEW_TOWER_ULS((prev) => [...prev, ''])
                                break;

                            case "NEW_TOWER":
                                setNEW_TOWER((prev) => [...prev, ''])
                                break;

                            case "NEW_SECTOR_ADDITION":
                                setNEW_SECTOR_ADDITION((prev) => [...prev, ''])
                                break;
                            ...
                            case "UPGRADE_SW_ONLY":
                                setUPGRADE_SW_ONLY((prev) => [...prev, ''])
                                break;

                            case "FDD_UPGRADE":
                                setFDD_UPGRADE((prev) => [...prev, ''])
                                break;

                            case "UPGRADE_ULS":
                                setUPGRADE_ULS((prev) => [...prev, ''])
                                break;

                            case "FDD_TWIN_BEAM":
                                setFDD_TWIN_BEAM((prev) => [...prev, ''])
                                break;

                            case "L900_UPGRADE":
                                setL900_UPGRADE((prev) => [...prev, ''])
                                break;

                            case "OEM_SWAP":
                                setOEM_SWAP((prev) => [...prev, ''])
                                break;

                            case "TDD_UPGRADE":
                                setTDD_UPGRADE((prev) => [...prev, ''])
                                break;

                            case "G2G_UPGRADE":
                                setG2G_UPGRADE((prev) => [...prev, ''])
                                break;

                            case "TDD_TWIN_BEAM":
                                setTDD_TWIN_BEAM((prev) => [...prev, ''])
                                break;

                            case "G2G_SEC_ADDITION":
                                setG2G_SEC_ADDITION((prev) => [...prev, ''])
                                break;

                            case "L2100_UPGRADE":
                                setL2100_UPGRADE((prev) => [...prev, ''])
                                break;
                            case "FDD_SEC_ADDITION":
                                setFDD_SEC_ADDITION((prev) => [...prev, ''])
                                break;
                            case "TDD_SEC_ADDITION":
                                setTDD_SEC_ADDITION((prev) => [...prev, ''])
                                break;
